...
이 문서는 저장소의 백서 초안이다. 과거 raw sweep, 초기 pair difference,
register-pressure 진단처럼 현재 coefficient 산출에 직접 쓰이지 않는 내용은
`archive/legacy_20260707/`로 이동했다. 과거 실험 흐름 전체는
`archive/legacy_20260707/docs/gpu_power_modeling_whitepaper_synthesis_history_ko.md`에 있다.

> **Protocol status, 2026-07-12:** 아래 RTX 3090 숫자는 2026-07-08 당시
> protocol에서 얻은 historical evidence다. 해당 strict CSV의 Global L1/L2
> `mode_pair`는 `*-clocked_empty`이고 NCU evidence에도 `global_addr_only`가 없다.
> 현행 protocol은 Global L1/L2/DRAM에 `global_addr_only` control,
> exact-coordinate control NCU acceptance, Tensor/DRAM pair-locked ITER를 요구한다.
> 따라서 아래 숫자를 현행 cross-platform strict final coefficient로 인용하면 안 되며
> RTX 3090 재측정이 필요하다.
...
대상 결과는 RTX 3090 finalplan, 2026-07-08 stability/strict 재측정분이다. 이 값은
당시 대표값이며 현행 protocol의 final 값은 아니다. 단위가 다르므로 Tensor는
pJ/FLOP, memory path는 pJ/bit로 읽어야 한다.
...
|---|---:|---|---:|---|---|
| Tensor MMA incremental | 0.129216 | pJ/FLOP | 6 | historical accepted | RF=8/16 당시 strict 후보. 현행 pair/control gate로 재실행 필요 |
| Shared scalar path | 0.170590 | pJ/bit | 6 | historical accepted | Shared pair는 현행과 같지만 전체 package 재실행 전 final 아님 |
| Global L1 hit path | 0.173483 | pJ/bit | 6 | historical provisional | treatment NCU path는 명확하지만 energy control이 `clocked_empty` |
| L2 CG hit path | 1.131073 | pJ/bit | 6 | historical provisional | treatment NCU path는 명확하지만 energy control이 `clocked_empty` |

현행 strict final component table은 Tensor/Shared scalar/Global L1/L2 네 row만
허용하지만, 위 과거 table이 새 gate를 통과했다는 뜻은 아니다. Register
direct/register-pressure와 DRAM streaming은 각각 control/proxy 또는 sanity path이며,
...
기존 수치 산출에 사용한 primary artifact는 finalplan energy run, 당시
matched-control 분석, NCU acceptance 결과다. 이 목록은 provenance이며 현행 protocol
충족 목록이 아니다.
...
2026-07-08 당시 coefficient를 얻은 sweep은 아래 조건이다. Global-memory control이
`clocked_empty`인 행은 현행 protocol과 다르다.
