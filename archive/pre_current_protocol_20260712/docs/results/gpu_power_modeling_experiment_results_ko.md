...
> **Protocol status, 2026-07-12:** 이 문서의 RTX 3090 수치와 그림은
> 2026-07-08 당시 protocol 결과를 정리한 historical report다. 기존 strict CSV의
> Global L1/L2 pair는 `global_l1_load_only/l2_cg_load_only - clocked_empty`이고,
> `global_addr_only` control의 동일 좌표 NCU acceptance가 없다. 현행 protocol은
> Global L1/L2/DRAM에 address control을 사용하고 Tensor/DRAM 동일 ITER 및 control
> NCU acceptance를 강제한다. 따라서 아래 값을 현행 strict final coefficient로
> 재사용하지 않으며 RTX 3090 재측정 전에는 historical/provisional로 읽는다.

이 문서는 방법론 설명이 아니라, 실제 실험 결과를 바로 확인하기 위한 정리 문서다. 모든 sweep 조건은 표로 정리했고, 단위가 필요한 값에는 단위를 명시했다.

## 1. 결과 한눈에 보기

![Historical component coefficients](../assets/component_energy_method/final_component_coefficients.png)

아래 값은 순수 회로/bitcell energy가 아니다. NVML board-level energy에서 control kernel을 뺀 뒤, NCU counter로 경로가 맞는지 검증한 effective microbenchmark coefficient다. 2026-07-08 stability/strict 재측정에서는 `energy_source=nvml_total_energy`, `energy_integration_method=total_energy_mj_delta`, `nvml_total_energy_supported=true` row만 최종값에 사용했다. RTX 3090의 `GetPowerUsage` fallback 의미는 `one_sec_average`지만, 아래 energy numerator는 endpoint power fallback이 아니다. 최신 strict+fresh NCU 결과는 [rtx3090_strict_scope_fresh_ncu_component_coefficients_20260708.md](../../../../results/summary/rtx3090_strict_scope_fresh_ncu_component_coefficients_20260708.md)에 따로 고정했고, reliability audit은 4개 component 모두 `accepted`다. 기존 current reporting 표에는 `measurement_scope` 컬럼 도입 이전 schema에서 추론한 결과도 남아 있으므로, 새 A100/V100/H100 및 새 finalplan run은 `--require-explicit-measurement-scope`를 통과해야 한다. GPU 세대별 power API 의미와 final/provisional/reject 기준은 [power_measurement_api_matrix_ko.md](../../../../docs/platforms/power_measurement_api_matrix_ko.md)를 따른다. 반복 run은 `nearest-control` pairing과 `delta_E >= 10 J`, `delta_E / max(E_treatment, E_control_scaled) >= 0.5%` gate를 적용했다. Power API audit 결과는 [rtx3090_finalplan_stability_power_api_audit_20260708.md](../../../../results/summary/rtx3090_finalplan_stability_power_api_audit_20260708.md)에 남겼고, 102개 raw energy row가 기존 audit 기준으로 `final_candidate`를 통과했다. Component reliability audit은 [rtx3090_finalplan_stability_component_reliability_audit_20260708.md](../../../../results/summary/rtx3090_finalplan_stability_component_reliability_audit_20260708.md)에 정리했다. Current sanity audit은 [rtx3090_current_component_sanity_audit_20260708.md](../../../../results/summary/rtx3090_current_component_sanity_audit_20260708.md)에 추가했고, 현재 보고값은 fail 0개와 warning 4개다.
...
![Strict measurement-scope coefficients](../assets/component_energy_method/rtx3090_strict_scope_component_coefficients.png)

![RTX 3090 strict coefficient summary](../assets/component_energy_method/rtx3090_strict_scope_component_coefficients_summary.png)

![RTX 3090 strict NCU evidence](../assets/component_energy_method/rtx3090_strict_scope_ncu_evidence.png)

과거 결과를 재현하거나 비교할 때는 아래 표를 사용한다. Tensor는 `pJ/FLOP`, memory path는 `pJ/bit`라
서로 직접 크기 비교하면 안 된다. 아래 값은 모두 순수 회로 에너지가 아니라
NCU로 treatment path를 검증한 board-level effective microbenchmark coefficient다.
현행 최종 보고값으로 승격하려면 새 control gate를 포함해 재실행해야 한다.
...
| 그림 | 파일 | 보여주는 내용 |
|---|---|---|
| 최종 component coefficient | `docs/assets/component_energy_method/final_component_coefficients.png` | primary coefficient bar, bootstrap CI, auxiliary median range summary |
| finalplan factor sweep | `docs/assets/component_energy_method/finalplan_factor_sweep_coefficients.png` | reuse/load_repeat 변화에 따른 coefficient 변화 |
| Tensor reuse factor trend | `docs/assets/component_energy_method/tensor_reuse_factor_trend.png` | RF=1,2,4,8,16 broad factor-exact NCU sweep의 Tensor MMA 증분 추이. RF1/2/4는 diagnostic으로 표시 |
| finalplan sweep 조건 | `docs/assets/component_energy_method/finalplan_sweep_design_matrix.png` | component별 W_SM, blocks/SM, active_SM, factor sweep 조건 |
| NCU hit-rate/access 검증 | `docs/assets/component_energy_method/ncu_hit_rate_validation.png` | L1/L2 hit rate, L1/L2/DRAM access count, byte traffic과 accepted/rejected path |
| Live NCU evidence fields | `docs/assets/component_energy_method/ncu_live_evidence_fields.png` | live NCU run에서 L1/L2/DRAM access count, bytes, shared bytes, long scoreboard, acceptance/status가 채워졌는지 확인 |
| NCU traffic 검증 | `docs/assets/component_energy_method/ncu_path_validation_bytes.png` | shared/L1/L2/DRAM byte traffic |
| 초기 Sweep 1 blocks/SM | `docs/assets/component_energy_method/sweep1_blocks_trend.png` | blocks/SM 변화에 따른 pJ/FLOP 추세. final coefficient 입력은 아니며 원본 CSV는 archive 보관 |
| 초기 Sweep 2 W_SM | `docs/assets/component_energy_method/sweep2_wsm_trend.png` | W_SM 변화에 따른 pJ/FLOP 추세. final coefficient 입력은 아니며 원본 CSV는 archive 보관 |

문서에 표시하는 이미지는 Markdown renderer 호환성을 위해 PNG를 사용한다. 원본 vector SVG는
같은 디렉터리에 동일한 basename으로 함께 보관한다.

![Finalplan factor sweep](../assets/component_energy_method/finalplan_factor_sweep_coefficients.png)
...
Energy run은 NCU 없이 NVML total energy counter로 실행했다. NCU는 별도 대표 조건에서 경로 검증에 사용했다. Power API 의미와 제약은 [power_measurement_api_matrix_ko.md](../../../../docs/platforms/power_measurement_api_matrix_ko.md)를 따른다.
...
| mode | 목적 | acceptance | L1 hit (%) | L2 hit (%) | access evidence | byte evidence | long SB (%) | 해석 |
|---|---|---|---:|---:|---|---|---:|---|
| `reg_mma` | Tensor treatment RF=4 | accepted | 34.959 | 77.580 | L1 0 sectors, L2 0 sectors, DRAM 1.998e6 sectors | L1 0 B, L2 8.029e7 B, DRAM 6.393e7 B | 0.0100 | HMMA 있음, spill/local 0. Memory traffic은 tensor threshold 이하 |
| `reg_operand_only` | Tensor control RF=4 | accepted | 31.189 | 26.033 | L1 0 sectors, L2 9.034e5 sectors, DRAM 3.191e6 sectors | L1 0 B, L2 1.876e8 B, DRAM 1.476e8 B | 0.00972 | HMMA 없음. control의 residual memory traffic도 threshold 이하 |
| `shared_scalar_load_only` | Shared scalar LR=4 | accepted | 21.075 | 42.076 | L1 0 sectors, L2 4.150e6 sectors, DRAM 5.729e6 sectors | Shared 5.374e11 B, L1 0 B, L2 4.058e8 B, DRAM 3.028e8 B | 0.00211 | shared bytes가 지배적이고 global/L2/DRAM traffic은 매우 작음 |
| `global_l1_load_only` | Global L1 LR=4 | accepted | 99.998 | 66.994 | L1 3.359e10 sectors, L2 5.661e6 sectors, DRAM 7.197e6 sectors | L1 1.075e12 B, L2 5.928e8 B, DRAM 4.527e8 B | 17.447 | L1 hit path로 인정. L2/DRAM byte leakage가 L1 bytes 대비 작음 |
| `l2_cg_load_only` | L2 CG LR=4 | accepted | 0.000006 | 99.898 | L1 1.679e10 sectors, L2 1.680e10 sectors, DRAM 1.192e7 sectors | L1 5.374e11 B, L2 5.380e11 B, DRAM 5.407e8 B | 867.454 | L1 hit은 사실상 0이고 L2 hit가 지배적. L1 sectors는 request-side traffic으로 해석 |
| `dram_cg_load_only` | DRAM sanity LR=4 | accepted | 0.000006 | 0.104 | L1 1.679e10 sectors, L2 1.680e10 sectors, DRAM 1.682e10 sectors | L1 5.374e11 B, L2 5.388e11 B, DRAM 5.386e11 B | 1747.880 | L1/L2 hit가 낮고 DRAM sectors/bytes가 충분히 커서 streaming sanity로 인정 |
| `l2_load_only` | L2 capacity 후보 | rejected | 88.369 | 99.794 | legacy LR4 row | L1 1.075e12 B, L2 1.254e11 B, DRAM 2.955e8 B | 70.728 | L1 hit가 너무 높아 L2 계수에서 제외 |
| `shared_load_only` | Shared WMMA load 후보 | rejected | 26.849 | 57.606 | legacy LR4 row | Shared 5.374e11 B, L1 0 B, L2 4.561e8 B, DRAM 3.195e8 B | 0.000554 | bank conflict가 높아 제외 |

이 표와 이미지는 hit rate만 보지 않는다. L1/L2/DRAM access count와 bytes가 같이 있어야
cache path가 실제로 의도대로 동작했다고 판단한다. L1 access는 NCU가 request counter를
제공하면 request, 아니면 sector로 표시하고, L2/DRAM access는 sector counter다.
`l2_cg_load_only`처럼 L1 sectors가 커도 L1 hit가 거의 0이면 L1에 hit된 것이 아니라
L1TEX request-side traffic으로 해석한다.

2026-07-09에는 같은 필드가 실제 NCU live run에서도 확인되는지 별도로 점검했다.
이 run은 coefficient 산출용 반복 실험이 아니라, `ncu_hit_rate_validation.png`와
acceptance report에 표시하는 필드가 실제 NCU output에서 채워지는지 검증한 것이다.
대표 RF/LR=4 row 6개가 모두 `pass`였다.
...
| 확인 항목 | 표의 열 |
|---|---|
| L1/L2/DRAM access count | `L1 accesses`, `L2 accesses`, `DRAM accesses` |
| L1/L2/DRAM bytes | `L1 bytes (B)`, `L2 bytes (B)`, `DRAM bytes (B)` |
| shared bytes | `shared bytes (B)` |
| long scoreboard stall | `long SB (%)` |
| acceptance/status | `acceptance`, `field check` |

| mode | acceptance | L1 hit (%) | L2 hit (%) | L1 accesses | L2 accesses | DRAM accesses | shared bytes (B) | L1 bytes (B) | L2 bytes (B) | DRAM bytes (B) | long SB (%) | field check |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| `reg_mma` | accepted | 36.2957 | 32.3856 | 0 sectors | 431231 sectors | 2.1648e6 sectors | 0 | 0 | 1.20292e8 | 9.38189e7 | 0.010564 | pass |
| `reg_operand_only` | accepted | 31.7291 | 63.2529 | 0 sectors | 427908 sectors | 2.11342e6 sectors | 0 | 0 | 1.2116e8 | 9.01961e7 | 0.009671 | pass |
| `shared_scalar_load_only` | accepted | 20.8079 | 15.0719 | 0 sectors | 724793 sectors | 3.04068e6 sectors | 5.37401e11 | 0 | 1.55649e8 | 1.19054e8 | 0.001967 | pass |
| `global_l1_load_only` | accepted | 99.9998 | 57.2715 | 3.35872e10 sectors | 41984 sectors | 4.3892e6 sectors | 0 | 1.07479e12 | 1.79393e8 | 1.40454e8 | 17.4343 | pass |
| `l2_cg_load_only` | accepted | 0.000007 | 99.9066 | 1.67936e10 sectors | 1.67994e10 sectors | 1.40017e7 sectors | 0 | 5.37395e11 | 5.38188e11 | 7.19515e8 | 864.97 | pass |
| `dram_cg_load_only` | accepted | 0.000007 | 0.038381 | 1.67936e10 sectors | 1.67998e10 sectors | 1.6818e10 sectors | 0 | 5.37395e11 | 5.38663e11 | 5.3847e11 | 1784.08 | pass |

Live check 산출물:

| artifact | path |
|---|---|
| NCU cache summary | `results/ncu/rtx3090_ncu_evidence_check_20260709/ncu_cache_validation_summary.csv` |
| NCU path acceptance | `results/summary/rtx3090_ncu_evidence_check_acceptance_20260709.md` |
| Evidence field check | `results/summary/rtx3090_ncu_evidence_live_field_check_20260709.md` |
...
아래 그림은 2026-07-08 factor-exact NCU broad sweep에서 RF=1,2,4,8,16을 모두
같은 축에 놓은 것이다. RF1/RF2/RF4는 Tensor MMA 증분 추세를 보기 위해 표시하지만,
반복 간 min-max spread가 커서 최종 대표값의 anchor로 쓰지는 않는다.
...
| 항목 | 의미 | 해석 방법 |
|---|---|---|
| x축 `reuse factor` | 같은 Tensor MMA benchmark 조건에서 logical MMA 반복 비중을 바꾸는 무차원 파라미터 | RF가 커질수록 register-fragment operand를 더 많이 재사용하는 조건에 가까워진다. 즉 operand 준비/control overhead 대비 Tensor MMA work 비중이 달라진다. |
| y축 `pJ/FLOP` | `reg_mma - reg_operand_only`의 board-level energy 차이를 NCU/실험 denominator의 FLOP로 나눈 값 | 값이 작을수록 해당 RF 조건에서 추가 Tensor MMA work 1 FLOP당 effective board-level 증분 에너지가 작다는 뜻이다. |
| 진한 선과 큰 점 | 각 RF에서 valid treatment-control row 3개의 median | 대표 추세만 보여준다. median line이 단조 감소/증가하지 않으면 RF에 따른 overhead, scheduling, clock/power-state, denominator sensitivity가 남아 있다고 본다. |
| 세로 whisker | 같은 RF의 min-max 범위 | whisker가 길면 반복 안정도가 낮다. RF1/RF2/RF4는 이 범위가 커서 final Tensor coefficient anchor로 쓰지 않는다. |
| 연한 점 | 각 RF의 개별 반복 결과 | outlier가 있는지 확인하는 용도다. RF2처럼 median은 낮지만 high outlier가 있으면 단일값으로 해석하면 안 된다. |

이 그림에서 볼 수 있는 것은 “Tensor MMA 증분 계수가 RF 설정에 민감하다”는 점이다.
RF4는 median이 `0.311 pJ/FLOP`로 높고, RF2는 median이 `0.082 pJ/FLOP`로 낮지만
high outlier가 있다. RF8/RF16은 targeted/duration-scaling follow-up과 비교할 수 있는
중간 구간이다. 따라서 이 broad sweep만으로 “Tensor Core 순수 회로 에너지는 특정
단일값이다”라고 결론 내리면 안 된다. 보고서에서는 RF8/RF16 targeted 및
duration-scaling 결과를 Tensor range의 주 근거로 사용하고, RF1/RF2/RF4는 추가
확장 실험이 필요한 diagnostic evidence로 둔다.

2026-07-08 factor-exact NCU broad RF 요약:

| reuse factor (unitless) | valid rows (count) | median (pJ/FLOP) | min (pJ/FLOP) | max (pJ/FLOP) | 해석 |
|---:|---:|---:|---:|---:|---|
| 1 | 3 | 0.225606 | 0.054161 | 0.345728 | diagnostic, spread 큼 |
| 2 | 3 | 0.081737 | 0.078486 | 0.359326 | diagnostic, high outlier 존재 |
| 4 | 3 | 0.310835 | 0.186392 | 0.359773 | diagnostic, high-side |
| 8 | 3 | 0.167926 | 0.099616 | 0.254906 | RF8 targeted/duration과 비교용 |
| 16 | 3 | 0.147404 | 0.120146 | 0.169745 | RF16 targeted/duration과 비교용 |

아래 표는 이전 2026-07-05 finalplan detail view다. 실행 흐름을 설명하기 위한
이력으로 남기며, 현재 RF trend 그래프는 위의 2026-07-08 factor-exact NCU summary를
사용한다.
...
![Sweep 1 blocks trend](../assets/component_energy_method/sweep1_blocks_trend.png)
...
![Sweep 2 W_SM trend](../assets/component_energy_method/sweep2_wsm_trend.png)
