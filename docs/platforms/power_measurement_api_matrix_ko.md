...
| V100 / GV100 | `nvcc fatal: Unsupported gpu architecture 'compute_70'` | CUDA 13 compiler가 Volta offline compilation을 지원하지 않음 | CUDA 12.x compiler를 선택하고 `nvcc --list-gpu-arch`에 `compute_70`이 있는지 확인한 뒤 clean rebuild |
| V100 / GV100 | NCU `gv100` query 실패 | denominator/path 검증이 안 되므로 pJ/bit path claim을 할 수 없다 | GV100 지원이 공식 확인된 Nsight Compute 2024.3을 우선 지정하고 `--list-chips`, `--query-metrics --chips gv100`을 재확인. 실패하면 NCU 미검증으로 분리 |
...
schema가 없었다. Tensor kernel/control revision도 현재 fixed-RF v4와 다르다. 따라서
...
| "GPU memory power = external-memory path pJ/bit" | memory subsystem power와 kernel external-read traffic denominator는 같은 레벨이 아니다 | "effective GPU-device read-path metadata로만 사용했으며 물리 HBM/GDDR energy가 아니다" |
