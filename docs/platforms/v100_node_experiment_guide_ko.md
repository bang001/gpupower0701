...
| NCU validation | mode, tensor %, L1 hit rate (%), L2 hit rate (%), L1 accesses (requests/sectors), L2 accesses (sectors), DRAM accesses (sectors), shared bytes, L1 bytes, L2 bytes, DRAM bytes, top stall %, status |
...
| NCU 기준 | 통과 조건 |
|---|---:|
| L2 CG | L1 hit <= 1%, L2 hit >= 95%, L2 access/bytes 존재, DRAM/L2 bytes <= 2% |
