...
NCU 단계에서 `ERR_NVGPUCTRPERM`이 발생하면 일반 사용자에게 GPU performance
counter 권한이 없는 상태다. 관리자에게 non-admin counter 접근을 열어 달라고 요청하는
것이 우선이고, sudo 권한이 있는 노드에서는 generated package를 다음처럼 재실행한다.
이 옵션은 NCU preflight/sidecar/goal-readiness 명령에만 적용되며, NVML energy sweep은
NCU와 분리된 상태로 유지된다.
...
### 4.0 RTX 3090/A100/V100 실행 package 한눈에 보는 비교
...
표가 아니며, A100/V100 수치는 아직 node-local NCU/power gate를 통과한
실측 결과가 아닌 **실행 계획**이다.

| 구분 | RTX 3090 / GA102 | A100 / GA100 | V100 / GV100 | 단위 및 조건 |
|---|---|---|---|---|
| CUDA / profile | `sm_86`, 82 SM | `sm_80`, 108 SM | `sm_70`, 80 SM | runtime SM 수가 profile과 다르면 preflight에서 중단 |
| energy blocks/SM | 8,16 | 16,32 | 1,2,4,8,16,32 | V100은 저밀도 구간까지 utilization sweep |
| Tensor | W2048; RF 1,2,4,8,16 | W2048; RF 1,2,4,8,16 | W2048; RF 1,2,4,8,16 | W: KiB/SM 고정 좌표, RF: count; 실제 RF footprint는 ptxas/NCU로 확인 |
| Shared scalar W | 32,64 | 64,128 | 32,64 | KiB/SM; shared residency 한도 통과 필수 |
| Global L1 유효 W/B | W8/B8; W16/B8,16 | W16/B16; W32/B16,32 | W8/B1,2,4,8; W16/B1,2,4,8,16; W32/B1,2,4,8,16,32 | W: KiB/SM, B: blocks/SM; block당 tile 1 KiB 미만 제외 |
| L2 CG 유효 W/B | W64/B8,16 | W16/B16; W32,64,128/B16,32 | W32,64/B1,2,4,8,16,32 | `ld.global.cg`; A100 W16/B32는 0.5 KiB/block이라 제외 |
| DRAM sanity | W8192; B8,16 | W8192; B16,32 | W8192; B1,2,4,8,16,32 | KiB/SM; 총 working set이 nominal L2보다 커야 함 |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
| candidate matrix | 92 | 128 | 348 | rows/1 repeat, feasibility 필터 전 |
| 유효 energy 좌표 | 86 | 116 | 330 | commands/1 repeat, treatment/control 모두 포함 |
| energy raw | 430 | 580 | 1,650 | rows, 유효 좌표 x 5 repeats; schema/revision smoke 3행 별도 |
| Tensor pair calibration | 10 coordinates / 20 commands | 10 coordinates / 20 commands | 30 coordinates / 60 commands | B x RF 좌표마다 treatment/control-floor calibration 2회; energy raw에 미포함 |
| primary NCU sidecar | 44 | 74 | 44 | cases; energy run과 분리, diagnostic mode 미포함 |
| strict NCU anchor B | 8 | 16 | 32 | blocks/SM; B가 다른 energy row를 채택하려면 exact-coordinate NCU 추가 필요 |
| power fallback 의미 | 1 s average | instantaneous | instantaneous | 최종 numerator는 모두 total-energy mJ delta 요구 |
| 핵심 플랫폼 제약 | GA102 16 blocks/SM profile, 6 MiB L2 | 40 MiB L2에서 4개 W plateau 후보 검증 | CUDA 12.x `compute_70` + `gv100` 지원 NCU 필수 | 조건 미충족 시 coefficient reject |
...
|---|---|---|---|---|
| CUDA arch | `sm_86` | `sm_80` | `sm_70` | V100은 `compute_70` 지원 CUDA 12.x 필요 |
| profile active SM | 82 | 108 | 80 | SMs, runtime/preflight 값이 우선 |
| profile max blocks/SM | 16 | 32 | 32 | blocks/SM |
| energy blocks/SM | 8,16 | 16,32 | 1,2,4,8,16,32 | blocks/SM |
| generated strict NCU blocks/SM | 8 | 16 | 32 | blocks/SM |
| Tensor W_SM | 2048 | 2048 | 2048 | KiB, register footprint 크기가 아닌 고정 좌표 |
| Tensor RF | 1,2,4,8,16 | 1,2,4,8,16 | 1,2,4,8,16 | reuse factor, unitless |
| Tensor ITER policy | RF별 treatment/control-floor dual calibration 후 pair-lock | 동일 | 동일 | 두 candidate ITER의 최대값을 두 mode에 동일 적용, direct net-energy subtraction |
| Tensor common inner op | RF당 dependent register integer add 1개 | 동일 | 동일 | treatment/control에 모두 포함해 차분에서 상쇄; control FP32 FMA/checksum/memory 없음 |
| Shared W_SM | 32,64 | 64,128 | 32,64 | KiB/SM |
| Global L1 W_SM | 8,16 | 16,32 | 8,16,32 | KiB/SM |
| L2 CG W_SM | 64 | 16,32,64,128 | 32,64 | KiB/SM |
| DRAM sanity W_SM | 8192 | 8192 | 8192 | KiB/SM = 8 MiB/SM |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
| Tensor NCU RF | 1,2,4,8,16 | 1,2,4,8,16 | 1,2,4,8,16 | unitless |
| Shared/L1/L2 NCU LR | 1,2,4,8,16 | 1,2,4,8,16 | 1,2,4,8,16 | count |
| DRAM NCU LR | 1,4,8,16 | 1,4,8,16 | 1,4,8,16 | count |
| measurement duration | 10 | 10 | 10 | s/energy command |
| energy repeats | 5 | 5 | 5 | count/valid coordinate |
| fallback power semantics | `one_sec_average` | `instant` | `instant` | final numerator는 모두 total-energy mJ delta 요구 |
| NCU chip | `ga102` | `ga100` | `gv100` | unavailable metric은 기록 후 필터, 필수 evidence 누락 시 reject |
...
| Component/path | RTX 3090 | A100 | V100 | 개수에 포함된 변화 축 |
|---|---:|---:|---:|---|
| Tensor | 20 / 100 | 20 / 100 | 60 / 300 | 2 modes x blocks/SM x RF 5개 |
| Shared scalar | 24 / 120 | 24 / 120 | 72 / 360 | 2 modes x W/B x LR 3개 |
| Global L1 | 18 / 90 | 18 / 90 | 90 / 450 | 2 modes x valid W/B x LR 3개 |
| L2 CG | 12 / 60 | 42 / 210 | 72 / 360 | 2 modes x valid W/B x LR 3개 |
| DRAM sanity | 12 / 60 | 12 / 60 | 36 / 180 | 2 modes x blocks/SM x LR 3개 |
| **합계** | **86 / 430** | **116 / 580** | **330 / 1,650** | schema/revision smoke와 NCU 제외 |

| package 규모 항목 | RTX 3090 | A100 | V100 | 단위/해석 |
|---|---:|---:|---:|---|
| candidate matrix rows/1 repeat | 92 | 128 | 348 | rows |
| feasibility로 제외된 matrix rows/1 repeat | 6 | 12 | 18 | rows, A100은 Global L1 6 + L2 CG 6, 나머지는 Global L1 최소 tile 조건 |
| 유효 energy 좌표/1 repeat | 86 | 116 | 330 | count |
| 예상 energy raw rows (`repeats=5`) | 430 | 580 | 1,650 | rows |
| schema/revision smoke | 3 | 3 | 3 | rows, 별도 CSV; full sweep 전 stale binary 거부 |
| Tensor pair calibration | 10 coordinates / 20 commands | 10 coordinates / 20 commands | 30 coordinates / 60 commands | blocks/SM x RF마다 2회 calibration; energy raw row에는 포함하지 않고 manifest 한 행/좌표로 기록 |
| primary NCU sidecar | 44 | 74 | 44 | cases, diagnostic mode 제외 |
| nominal energy kernel time | 4,300 | 5,800 | 16,500 | s, raw rows x 10 s 기준. Dual calibration의 control candidate가 크면 Tensor treatment가 연장되며 calibration/launch/NCU/분석 시간도 별도 |

RTX 3090/V100의 NCU 44 cases는 `clocked_empty` 1개, Tensor treatment/control
...
| Component/path | RTX 3090 generated plan | A100 generated plan | V100 generated plan | 단위 |
|---|---|---|---|---|
| Tensor | W2048/B8 | W2048/B16 | W2048/B32 | KiB/SM, blocks/SM |
| Shared scalar | W64/B8 | W128/B16 | W32/B32 | KiB/SM, blocks/SM |
| Global L1 | W8/B8 | W16/B16 | W32/B32 | KiB/SM, blocks/SM |
| L2 CG | W64/B8 | W16,32,64,128/B16 | W32/B32 | KiB/SM, blocks/SM; A100은 후보 set |
| DRAM sanity | W8192/B8 | W8192/B16 | W8192/B32 | KiB/SM, blocks/SM |
...
| capacity check | RTX 3090 | A100 | V100 | 해석 |
|---|---:|---:|---:|---|
| Shared 예약 근사 `W_SM+B` / shared profile | 72/100 KiB (72.0%) | 144/164 KiB (87.8%) | 64/96 KiB (66.7%) | Shared strict point의 profile 한도 대비 위치 |
| Global L1 W_SM / combined L1-shared | 8/128 KiB (6.3%) | 16/192 KiB (8.3%) | 32/128 KiB (25.0%) | shared를 할당하지 않는 global L1 후보의 per-SM footprint |
| L2 full-GPU W / nominal L2 | 5.125/6 MiB (85.4%) | 1.688-13.5/40 MiB (4.2-33.8%) | 2.5/6 MiB (41.7%) | A100은 4개 W에서 NCU-accepted plateau를 선택 |
| DRAM full-GPU W | 656 MiB | 864 MiB | 640 MiB | 모두 nominal L2보다 충분히 큰 streaming sanity footprint |
...
| DRAM sanity | L1 hit <= 1%, L2 hit <= 5%, DRAM access/bytes가 충분하고 dominant |
