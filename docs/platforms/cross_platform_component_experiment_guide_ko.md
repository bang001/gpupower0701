...
memory pair의 no-input-load control이며 global-load L1 byte 0을 확인한다. SMID verification
...
| Global L1 | L1 hit >= 95%, L1 access/bytes가 존재, L2/L1 byte ratio <= 1%, DRAM/L1 byte ratio <= 1% |
| L2 hit | L2 hit >= 95%, L2 access/bytes 존재, L1 bytes/L2 bytes <= 1%, DRAM/L2 byte ratio <= 2%. aggregate L1 hit는 `.cg` 진단값 |
