...
| general energy blocks/SM | 8,16 | general 16,32; L2 selected 1/2/4/8/16 | general 4,16,32; L2 selected 4/16/32 | 16,32 | A100/V100 L2는 precheck가 고른 B 하나만 energy/minimal NCU에 사용 |
...
| Global L1 유효 W/B | W8/B8; W16/B8,16 | W16/B16; W32/B16,32 | W8/B4; W16/B4,16; W32/B4,16,32 | W16/B16; W32/B16,32 | W: KiB/SM, B: blocks/SM; block당 tile 1 KiB 미만 제외 |
| L2 CG 유효 W/B | W64/B8,16 | candidate W16,32,64,128/B16,8,4 | candidate W32,64/B32,16,4 | W64,128/B16,32 | A100/V100은 두 anchor strict-pass 후보 하나만 실행 |
| External-memory read | W256,512,1024,2048; B8,16 | W2048,4096,8192; B16,32 | W256,512,1024,2048; B4,16,32 | W2048,4096,8192; B16,32 | KiB/SM; nominal L2 배수 sweep |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
| candidate matrix | 92 | 104 | 150 | 104 | A100/V100 L2 matrix는 selected B 하나 기준 |
| 유효 energy 좌표 | 86 | 98 | 132 | 98 | commands/1 repeat, treatment/control 모두 포함 |
| energy raw | 430 | 490 | 660 | 490 | rows, 유효 좌표 x 5 repeats; schema smoke 3행 별도 |
| Tensor pair calibration | 10 coordinates / 20 commands | 10 coordinates / 20 commands | 15 coordinates / 30 commands | 10 coordinates / 20 commands | B x RF 좌표마다 treatment/control-floor calibration 2회; energy raw에 미포함 |
| L2 pair calibration | 6 coordinates / 12 commands | 12 coordinates / 24 commands | 6 coordinates / 12 commands | 12 coordinates / 24 commands | A100/V100은 selected B의 W/LR만 calibration |
| primary NCU sidecar | 49 | 79 | 59 | 49 | full non-L2 39 + minimal L2 10/40/20/10 cases; selector precheck A100 최대 56/V100 최대 16 cases 별도 |
| strict NCU anchor B | 8 | Shared/L1 16; L2 selected 1/2/4/8/16 | Shared/L1 32; L2 selected 4/16/32 | 16 | blocks/SM |
...
shared bytes/block이 함께 증가하는지 확인한다. V100 B4/B16은 utilization 민감도 점이고
B32가 strict anchor다. final cross-platform 비교는 각 GPU의
...
| `.cg` W에서 L1 hit가 사라지고 final L2 service plateau | L1 <=1%, architecture-specific final L2 service >=95%; GA100은 source+fabric logical hit | L2 후보 |
...
V100 precheck W32/W64는 약 42/83%, A100 W16은 약 4%, H100 W64는 약 17%다. A100은 네 W에서
...
|---|---:|---:|---:|---:|---:|---|
| RTX 3090 | N/A (CLI 1) | 32,64 | 8,16 | 64 with `l2_cg_load_only` | 8192 | Tensor B4/8/16; general energy 8,16 |
| V100 | N/A (CLI 1) | 32,64 | 8,16,32 | 32,64 with `l2_cg_load_only` | 8192 | Tensor/general B4/16/32; L2 selected B32/16/4 |
| A100 | N/A (CLI 1) | 64,128 | 16,32 | 16,32,64,128 with `l2_cg_load_only` | 8192 | Tensor B4/16/32; general energy 16,32 |
| H100 | N/A (CLI 1) | 64,128 | 16,32 | 64,128 with `l2_cg_load_only` | 8192 | Tensor B4/16/32; general energy 16,32 |
...
one warp/block이므로 B32도 최대 64 warps/SM의 50%에 해당한다. B4/B16은 energy
utilization sensitivity sweep이고, final coefficient에는 exact-coordinate NCU evidence가 있는 row만
사용한다.
...
| general energy blocks/SM | 8,16 | 16,32 | 4,16,32 | 16,32 | blocks/SM; Tensor는 아래 별도 sweep |
| Tensor blocks/SM | 4,8,16 | 4,16,32 | 4,16,32 | 4,16,32 | blocks/SM; energy와 NCU 동일 B 목록 |
| generated strict NCU blocks/SM | Tensor 4/8/16; Shared/L1 8 | Tensor 4/16/32; Shared/L1 16; L2 selected | Tensor 4/16/32; Shared/L1 32; L2 selected | Tensor 4/16/32; Shared/L1 16 | blocks/SM |
...
| External-memory W_SM | 256,512,1024,2048 | 2048,4096,8192 | 256,512,1024,2048 | 2048,4096,8192 | KiB/SM; L2 배수 sweep |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
...
| V100 | W8/B4; W16/B4,16; W32/B4,16,32 | W8/B16, W8/B32, W16/B32 | 1 KiB/block 미만 |
...
| A100 | selector candidate W16,32,64,128/B16,8,4,2,1 | 없음 | W16/W128 anchor를 통과한 B 하나만 full energy에 전달 |
...
|---|---:|---:|---:|---:|---|
| Tensor | 20 / 100 | 20 / 100 | 30 / 150 | 20 / 100 | 2 modes x blocks/SM x RF 5개 |
| Shared scalar | 24 / 120 | 24 / 120 | 36 / 180 | 24 / 120 | 2 modes x W/B x LR 3개 |
| Global L1 | 18 / 90 | 18 / 90 | 36 / 180 | 18 / 90 | 2 modes x valid W/B x LR 3개 |
| L2 CG | 12 / 60 | 24 / 120 | 12 / 60 | 24 / 120 | A100/V100은 2 modes x W x selected B x LR 3개 |
| External-memory read | 48 / 240 | 36 / 180 | 72 / 360 | 36 / 180 | 2 modes x W x blocks/SM x LR 3개 |
| **합계** | **86 / 430** | **98 / 490** | **132 / 660** | **98 / 490** | schema smoke와 NCU 제외 |

| package 규모 항목 | RTX 3090 | A100 | V100 | H100 | 단위/해석 |
|---|---:|---:|---:|---:|---|
| candidate matrix rows/1 repeat | 92 | 104 | 150 | 104 | rows; A100/V100 L2 selected B 반영 |
| feasibility로 제외된 matrix rows/1 repeat | 6 | 6 | 18 | 6 | rows, Global L1 최소 tile 조건 |
| 유효 energy 좌표/1 repeat | 86 | 98 | 132 | 98 | count |
| 예상 energy raw rows (`repeats=5`) | 430 | 490 | 660 | 490 | rows |
| schema/revision smoke | 3 | 3 | 3 | 3 | rows, 별도 CSV; full sweep 전 stale binary 거부 |
| Tensor pair calibration | 10 coordinates / 20 commands | 10 coordinates / 20 commands | 15 coordinates / 30 commands | 10 coordinates / 20 commands | blocks/SM x RF마다 2회 calibration; energy raw row에는 포함하지 않고 manifest 한 행/좌표로 기록 |
| L2 pair calibration | 6 coordinates / 12 commands | 12 coordinates / 24 commands | 6 coordinates / 12 commands | 12 coordinates / 24 commands | selected B의 W/LR마다 2회 calibration |
| primary NCU sidecar | 49 | 79 | 59 | 49 | cases; full non-L2와 minimal L2의 합, selector precheck 별도 |
| nominal energy kernel time | 4,300 | 4,900 | 6,600 | 4,900 | s, raw rows x 10 s 기준; calibration/launch/NCU/precheck 별도 |

비-L2 full profile은 39 cases이고, L2 minimal profile은 RTX/H100 10, A100 40,
V100 20 cases이므로 총 49/79/59/49 cases다. 이 수에는 selector의 조기
precheck case가 포함되지 않는다.
`INCLUDE_DIAGNOSTIC_NCU=1`을 켜면 이 수보다 증가하므로 보고서에 별도로 표기한다.

V100의 일반 component는 B4/B16/B32 세 점을 실행한다. L2는 precheck가 선택한 B 하나만
energy/minimal NCU에 사용하며, raw row는 660개다.
...
| L2 CG | W64/B8 | W16,32,64,128/selected B16,8,4 | W32,64/selected B32,16,4 | W64/B16 | KiB/SM, blocks/SM; A100/V100은 precheck-selected |
| External-memory anchor | W2048/B8 | W8192/B16 | W2048/B32 | W8192/B16 | KiB/SM, blocks/SM; 전체 W sweep 중 큰 anchor |
...
| L2 full-GPU W / nominal L2 | 5.125/6 MiB (85.4%) | 1.688-13.5/40 MiB (4.2-33.8%) | 2.5-5/6 MiB (41.7-83.3%) | 8.25/50 MiB (16.5%) | A100/V100은 두 anchor 이상을 precheck; H100 W128은 energy 보조점 |
| External-memory full-GPU W | 656 MiB | 864 MiB | 640 MiB | 1,056 MiB | 모두 nominal L2보다 큰 외부 read-path anchor footprint; 최종값은 architecture별 W sweep과 NCU gate로 선정 |
