...
| Shared scalar path | `shared_scalar_load_only` | `shared_scalar_addr_only` | NCU shared read bytes + access count | 같은 shared allocation/index loop 대비 scalar read path |
...
| External-memory read path | `dram_cg_load_only` | `global_addr_only` | NCU DRAM read/write bytes + expected source bytes | effective GPU-device path; not physical memory energy |

모든 profile의 Tensor와 memory final pair는 동일 ITER를 강제한다. treatment의 목표
측정시간과 `global_addr_only`의 control-duration floor를 각각 calibration하고, 두
candidate 중 큰 ITER를 treatment/control 모두에 적용한다. 이후
`delta_E = net_E_treatment - net_E_control`로 직접 차분한다. 서로 다른 ITER를 실행한
control power를 treatment 시간으로 늘려 빼는 duration-scaled 계산은 현행
cross-platform coefficient에서 허용하지 않는다. NCU hit-rate acceptance와 동일-ITER
energy gate는 서로 대체할 수 없으며 둘 다 통과해야 한다.
...
`accepted`, `accepted_with_caution`, `accepted_low_stability`, `accepted_effective_path`,
...
- Tensor static audit은 PATH의 다른 CUDA 버전을 사용하지 않도록,
  선택한 `NVCC` 실행 파일과 같은 `bin/` 디렉터리의 `cuobjdump`를
  기본으로 사용한다. 필요하면 `CUOBJDUMP=/absolute/path/cuobjdump`로
  명시한다.
- `schema_revision_smoke` 내부는 kernel execution, power/schema audit,
  Tensor binary audit으로 분리되며, 실패 명령은
  `PIPELINE_COMMAND_FAILED` 행에 stage/label/return code를 남긴다. 두 서버의
  로그가 같은 상위 stage에서 멈춰도 label과 audit `reasons`가 다르면
  같은 근본 원인으로 판정하지 않는다.
- 생성 shell의 전역 `ERR` trap은 나머지 stage의 오류도
  `PIPELINE_ABORT` 행에 stage/line/return code/command로 남긴다.
...
| memory-path energy blocks/SM | 8 | 16; L2 selected 1/2/4/8/16 | 32; L2 selected 4/16/32 | 16; L2 selected 8/16 | energy와 exact NCU가 같은 단일 B; Tensor는 별도 sweep |
| Tensor | B4/8/16; RF 1,2,4,8,16 | B4/16/32; RF 1,2,4,8,16 | B4/16/32; RF 1,2,4,8,16 | B4/16/32; RF 1,2,4,8,16 | W=N/A, CLI placeholder 1 KiB; B=blocks/SM; RF=count |
| Shared scalar W/B | W64/B8 | W128/B16 | W32/B32 | W128/B16 | KiB/SM; shared residency 한도 통과 필수 |
| Global L1 W/B | W8/B8 | W16/B16 | W32/B32 | W16/B16 | W: KiB/SM, B: blocks/SM |
| L2 CG W/B | W32,64/B8 | candidate W16,128/selected B | candidate W32,64/selected B | candidate W64,128/selected B | 두 W 모두 strict-pass한 후보 하나만 energy/final NCU에 사용 |
| External-memory read | W256,512,2048/B8 | W2048,4096,8192/B16 | W256,512,2048/B32 | W2048,4096,8192/B16 | KiB/SM; nominal L2보다 큰 low/mid/high 세 점 |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
| candidate/valid matrix | 72 / 72 | 72 / 72 | 72 / 72 | 72 / 72 | rows/1 repeat; strict defaults에는 NCU 없는 energy 좌표가 없음 |
| energy raw | 360 | 360 | 360 | 360 | rows, 72 commands x 5 repeats; schema smoke 3행 별도 |
| Tensor pair calibration | 15 coordinates / 30 commands | 동일 | 동일 | 동일 | B 3 x RF 5; treatment/control-floor calibration 2회 |
| L2 pair calibration | 6 coordinates / 12 commands | 동일 | 동일 | 동일 | W 2 x selected/fixed B 1 x LR 3 |
| primary NCU sidecar | 73 | 73 | 73 | 73 | full Tensor/Shared/L1 43 + minimal L2 12 + minimal external 18; selector precheck 별도 |
| strict NCU anchor B | memory 8 | memory 16; L2 selected 1/2/4/8/16 | memory 32; L2 selected 4/16/32 | memory 16; L2 selected 8/16 | blocks/SM |
| power fallback 의미 | 1 s average | instantaneous | instantaneous | 1 s average | 최종 numerator는 모두 total-energy mJ delta 요구 |
| 핵심 플랫폼 제약 | GA102 16 blocks/SM profile, 6 MiB L2 | 40 MiB partition-fabric L2 | CUDA 12.x `compute_70` + `gv100` 지원 NCU 필수 | SXM5 132-SM profile, 50 MiB partition-fabric L2, Tensor는 WMMA compatibility path | 조건 미충족 시 coefficient reject |
...
shared bytes/block이 함께 증가하는지 확인한다. 세 B점은 Tensor utilization 민감도에만
유지하고, memory path는 profile별 단일 B anchor를 쓴다. final cross-platform 비교는 각 GPU의
...
| `.cg` W에서 L1 hit가 사라지고 final L2 service plateau | L1 <=1%, architecture-specific final L2 service >=95%; GA100/GH100은 source+fabric logical hit | L2 후보 |
| full-GPU W가 L2의 여러 배이고 external read traffic 우세 | final L2 hit <=10%, DRAM read/L2 read >=90%, write/read <=1% | external-memory effective-path 후보 |
...
대표 좌표의 capacity 비율도 플랫폼마다 동일하지 않다. RTX L2 W32/W64는 6 MiB의 약
43/85%, V100 W32/W64는 약 42/83%, A100 W16/W128은 약 4/34%, H100 W64/W128은
약 17/33%다. 두 endpoint의 NCU plateau를 모두 확인하므로 한 점의 capacity 비율만
비교해서는 안 된다. 향후 동일한
...
| V100 | N/A (CLI 1) | 32 | 32 | 32,64 with `l2_cg_load_only` | 256,512,2048 | Tensor B4/16/32; memory B32; L2 selected B32/16/4 |
| A100 | N/A (CLI 1) | 128 | 16 | 16,128 with `l2_cg_load_only` | 2048,4096,8192 | Tensor B4/16/32; memory B16; L2 selected B16/8/4/2/1 |
| H100 SXM5 profile | N/A (CLI 1) | 128 | 16 | 64,128 with `l2_cg_load_only` | 2048,4096,8192 | Tensor B4/16/32; memory B16; L2 selected B16/8 |

Tensor mode에는 memory `W_SM`이 적용되지 않는다. 현재 `W_SM=1 KiB`는 CLI
parser/coordinate placeholder일 뿐이며, 실제 register footprint는 ptxas register count,
threads/block, resident blocks/SM로 판단한다.

모든 기본 memory 좌표는 block당 최소 1 KiB tile을 만족하며 energy와 NCU가 정확히
일치한다. broad W/B discovery가 필요하면 별도 tag로 실행하고 strict summary에 섞지 않는다.

V100 strict 좌표는 Shared `W32/B32`, Global L1 `W32/B32`다. L2 CG는
W32/W64에서 normal contiguous B32와 sm-interleaved B32/B16/B4를 검사해 첫
strict-pass B를 선택한다. 두 working set은 2.5/5 MiB다. 현재 kernel은
one warp/block이므로 B32도 최대 64 warps/SM의 50%에 해당한다. B4/B16은 L2 selector와
Tensor sweep에만 남으며, final coefficient에는 exact-coordinate NCU evidence가 있는 row만 사용한다.
...
| L2 pair calibration | 각 유효 W/B/LR에서 `l2_cg_load_only` treatment 목표와 `global_addr_only` control 최소시간을 각각 calibration | coordinates/commands, 두 candidate ITER의 최대값을 양쪽에 공급하며 raw energy row가 아님 |
...
| memory-path energy blocks/SM | 8 | 16 | 32 | 16 | blocks/SM; energy와 exact NCU 공통 anchor |
| Tensor blocks/SM | 4,8,16 | 4,16,32 | 4,16,32 | 4,16,32 | blocks/SM; energy와 NCU 동일 B 목록 |
| generated strict NCU blocks/SM | Tensor 4/8/16; memory 8 | Tensor 4/16/32; memory 16; L2 selected | Tensor 4/16/32; memory 32; L2 selected | Tensor 4/16/32; memory 16; L2 selected | blocks/SM |
| Tensor W_SM | N/A (CLI 1) | N/A (CLI 1) | N/A (CLI 1) | N/A (CLI 1) | memory/register working set이 아닌 parser placeholder |
...
| Tensor source shape | 단일 A/B/C fragment, dependent scalar update, in-place A-sign flip, C epilogue | 동일 | 동일 | 동일 | treatment만 MMA; ptxas register footprint는 GPU/role별로 다름 |
| Shared W_SM | 64 | 128 | 32 | 128 | KiB/SM |
| Global L1 W_SM | 8 | 16 | 32 | 16 | KiB/SM |
| L2 CG W_SM | 32,64 | 16,128 | 32,64 | 64,128 | KiB/SM |
| L2 CG ITER policy | W/B/LR별 treatment/control-floor dual calibration 후 pair-lock | 동일 | 동일 | 동일 | 동일 ITER, direct net-energy subtraction |
| External-memory W_SM | 256,512,2048 | 2048,4096,8192 | 256,512,2048 | 2048,4096,8192 | KiB/SM; L2보다 큰 low/mid/high sweep |
| memory energy LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | load repeat, count |
| Tensor NCU RF | 1,2,4,8,16 | 1,2,4,8,16 | 1,2,4,8,16 | 1,2,4,8,16 | unitless |
| Shared/L1/L2 NCU LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | count; energy와 exact match |
| DRAM NCU LR | 4,8,16 | 4,8,16 | 4,8,16 | 4,8,16 | count; energy와 exact match |
...
B 단위는 blocks/SM이다. 기본 profile은 invalid 조합을 만들지 않는다.

| GPU | 유효 Global L1 W/B | 제외되는 W/B | 제외 이유 |
|---|---|---|---|
| RTX 3090 | W8/B8 | 없음 | exact NCU anchor 한 점 |
| A100 | W16/B16 | 없음 | exact NCU anchor 한 점 |
| V100 | W32/B32 | 없음 | exact NCU anchor 한 점 |
| H100 | W16/B16 | 없음 | exact NCU anchor 한 점 |

L2 CG도 같은 최소 tile 조건을 적용한다. 두 W endpoint를 같은 selected/fixed B에서
검증하므로 Global L1 표와 별도로 다음 유효 좌표를 사용한다.
...
|---|---:|---:|---:|---:|---|
| Tensor | 30 / 150 | 30 / 150 | 30 / 150 | 30 / 150 | 2 modes x blocks/SM 3개 x RF 5개 |
| Shared scalar | 6 / 30 | 6 / 30 | 6 / 30 | 6 / 30 | 2 modes x W/B 1개 x LR 3개 |
| Global L1 | 6 / 30 | 6 / 30 | 6 / 30 | 6 / 30 | 2 modes x W/B 1개 x LR 3개 |
| L2 CG | 12 / 60 | 12 / 60 | 12 / 60 | 12 / 60 | 2 modes x W 2개 x selected/fixed B x LR 3개 |
| External-memory read | 18 / 90 | 18 / 90 | 18 / 90 | 18 / 90 | 2 modes x W 3개 x B 1개 x LR 3개 |
| **합계** | **72 / 360** | **72 / 360** | **72 / 360** | **72 / 360** | schema smoke와 NCU 제외 |

| package 규모 항목 | RTX 3090 | A100 | V100 | H100 | 단위/해석 |
|---|---:|---:|---:|---:|---|
| candidate matrix rows/1 repeat | 72 | 72 | 72 | 72 | rows; exact NCU coverage와 동일 |
| feasibility로 제외된 matrix rows/1 repeat | 0 | 0 | 0 | 0 | rows; 기본 profile 기준 |
| 유효 energy 좌표/1 repeat | 72 | 72 | 72 | 72 | count |
| 예상 energy raw rows (`repeats=5`) | 360 | 360 | 360 | 360 | rows |
| schema/revision smoke | 3 | 3 | 3 | 3 | rows, 별도 CSV; full sweep 전 stale binary 거부 |
| Tensor pair calibration | 15 coordinates / 30 commands | 동일 | 동일 | 동일 | blocks/SM x RF마다 2회 calibration |
| Shared/L1 pair calibration | 각 3 coordinates / 6 commands | 동일 | 동일 | 동일 | W/B 1 x LR 3 |
...
| primary NCU sidecar | 73 | 73 | 73 | 73 | cases; full Tensor/Shared/L1 43 + minimal L2 12 + minimal external 18 |
| 최대 L2 precheck | 0 | 56 | 16 | 24 | cases; 첫 pass에서 조기 종료 가능 |
| nominal energy kernel time | 3,600 | 3,600 | 3,600 | 3,600 | s, raw rows x 10 s 기준; calibration/launch/NCU/precheck 별도 |

full Tensor/Shared/L1 profile은 43 cases, L2 minimal은 12 cases, external-memory minimal은
18 cases이므로 모든 profile의 최종 sidecar는 73 cases다. 이 수에는 selector의 조기
precheck case가 포함되지 않는다.
`INCLUDE_DIAGNOSTIC_NCU=1`을 켜면 이 수보다 증가하므로 보고서에 별도로 표기한다.

V100의 B4/B16/B32는 Tensor에만 유지한다. Memory path는 B32 한 점이며 L2는 precheck가
선택한 B 하나만 energy/minimal NCU에 사용한다. 기본 raw row는 360개다.
...
|---|---|---|---|---|---|
| Tensor | W N/A (CLI1)/B4,8,16 | W N/A (CLI1)/B4,16,32 | W N/A (CLI1)/B4,16,32 | W N/A (CLI1)/B4,16,32 | Tensor NCU는 모든 energy B/RF 좌표 수집 |
| Shared scalar | W64/B8 | W128/B16 | W32/B32 | W128/B16 | KiB/SM, blocks/SM |
| Global L1 | W8/B8 | W16/B16 | W32/B32 | W16/B16 | KiB/SM, blocks/SM |
| L2 CG | W32,64/B8 | W16,128/selected B16,8,4,2,1 | W32,64/selected B32,16,4 | W64,128/selected B16,8 | KiB/SM, blocks/SM; A100/V100/H100은 precheck-selected |
| External-memory sweep | W256,512,2048/B8 | W2048,4096,8192/B16 | W256,512,2048/B32 | W2048,4096,8192/B16 | KiB/SM, blocks/SM; low/mid/high 세 점 모두 exact NCU |
...
| L2 full-GPU W / nominal L2 | 2.563-5.125/6 MiB (42.7-85.4%) | 1.688-13.5/40 MiB (4.2-33.8%) | 2.5-5/6 MiB (41.7-83.3%) | 8.25-16.5/50 MiB (16.5-33.0%) | 두 endpoint 모두 NCU plateau 필요 |
| External-memory full-GPU W | 20.5-164 MiB | 216-864 MiB | 20-160 MiB | 264-1,056 MiB | 모두 nominal L2보다 큰 세 footprint; 최종값은 NCU gate와 안정성으로 선정 |

Tensor의 CLI W1은 위 capacity 계산 대상이 아니다. Tensor/register mode의 실제
...
|---|---|---|---|
| Tensor | W N/A(legacy CLI W2048), B16, active SM 82 | RF8, RF16 | superseded v2; current coefficient 아님 |
...
| External-memory read path | architecture별 W sweep, NCU read-byte strict gate | 별도 effective-path 결과 | strict 4-component summary에는 미포함; 물리 DRAM 값으로 표기 금지 |

플랫폼 표는 target-node accepted coefficient가 아니라 실행 계획이다. 실제
...
| L2 candidate | `runtime active_SM x W_SM <= nominal L2` | L2 mode 실행 제외 또는 external-memory candidate로 분류 |
...
| L2 hit | `ncu_metric_profile=l2_path_minimal`, observed/expected source L2 bytes=0.95-1.05, L1 path hit<=1%, L1 hit/request<=1%, DRAM-read/source-read<=2%. GA100은 source+`srcunit_ltcfabric` logical final hit>=95%, source/fabric 보존=0.98-1.02, native-model 오차<=2 pp를 요구하며 direct/native 자체에는 95%를 강제하지 않는다. 다른 profile은 architecture별 direct/native 정책을 사용하고 V100은 native metric이 없을 때만 교차검증을 생략. `.cg`의 L1TEX request bytes 자체는 실패가 아님 |
| External-memory read | L1 hit <=1%, final L2 hit <=10%, source/expected 0.95-1.05, DRAM read/source >=0.90, write/read <=0.01, direct `dram__bytes_read.sum`/`dram__bytes_write.sum` source |
...
현재 공통 NCU runner의 기본값은 `application replay + cache-control none`이다. 따라서
metric pass마다 application과 warm-up이 다시 실행되며, case manifest에 replay/cache
정책과 metric profile이 기록된다. L2 hard gate는 작은 `l2_path_minimal` bundle로
수집하고, 비-L2 full summary와 행 단위로 merge한다. `hit+miss/read` 보존성이
0.98-1.02가 아니면 hit rate를 사용하지 않는다. A100은 normal `.cg` policy가 strict L2 hit를 충족하지 못할 때만
persisting-L2 access-policy window를 별도 진단한다. V100은 persistence control을 지원하지
않으며, RTX 3090/H100도 표준 결과에 persisting 값을 자동 혼합하지 않는다.
...
| External-memory read | `effective GPU-device external-memory read path`; physical HBM/GDDR energy라고 쓰지 않음 |
