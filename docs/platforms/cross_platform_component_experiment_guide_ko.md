...
| energy blocks/SM | 8,16 | 16,32 | 4,16,32 | 16,32 | V100은 B4/B16 민감도와 strict B32 비교 |
...
| L2 CG 유효 W/B | W64/B8,16 | W16/B16; W32,64,128/B16,32 | W32,64/B4,16,32 | W64,128/B16,32 | `ld.global.cg`; A100 W16/B32는 0.5 KiB/block이라 제외 |
...
| candidate matrix | 92 | 128 | 174 | 104 | rows/1 repeat, feasibility 필터 전 |
| 유효 energy 좌표 | 86 | 116 | 156 | 98 | commands/1 repeat, treatment/control 모두 포함 |
| energy raw | 430 | 580 | 780 | 490 | rows, 유효 좌표 x 5 repeats; schema/revision smoke 3행 별도 |
...
| L2 pair calibration | 6 coordinates / 12 commands | 21 coordinates / 42 commands | 18 coordinates / 36 commands | 12 coordinates / 24 commands | 유효 W/B/LR마다 treatment/control-floor calibration 2회; energy raw에 미포함 |
...
| V100 | W32,64/B4,16,32 | 없음 | 모든 좌표가 1 KiB/block 이상 |
...
| L2 CG | 12 / 60 | 42 / 210 | 36 / 180 | 24 / 120 | 2 modes x valid W/B x LR 3개 |
| DRAM sanity | 12 / 60 | 12 / 60 | 18 / 90 | 12 / 60 | 2 modes x blocks/SM x LR 3개 |
| **합계** | **86 / 430** | **116 / 580** | **156 / 780** | **98 / 490** | schema/revision smoke와 NCU 제외 |

| package 규모 항목 | RTX 3090 | A100 | V100 | H100 | 단위/해석 |
|---|---:|---:|---:|---:|---|
| candidate matrix rows/1 repeat | 92 | 128 | 174 | 104 | rows |
| feasibility로 제외된 matrix rows/1 repeat | 6 | 12 | 18 | 6 | rows, A100은 Global L1 6 + L2 CG 6, 나머지는 Global L1 최소 tile 조건 |
| 유효 energy 좌표/1 repeat | 86 | 116 | 156 | 98 | count |
| 예상 energy raw rows (`repeats=5`) | 430 | 580 | 780 | 490 | rows |
...
| L2 pair calibration | 6 coordinates / 12 commands | 21 coordinates / 42 commands | 18 coordinates / 36 commands | 12 coordinates / 24 commands | 유효 W/B/LR마다 2회 calibration; 동일 resolved ITER를 treatment/control에 적용 |
| primary NCU sidecar | 44 | 74 | 44 | 44 | cases, diagnostic mode 제외 |
| nominal energy kernel time | 4,300 | 5,800 | 7,800 | 4,900 | s, raw rows x 10 s 기준. Dual calibration의 control candidate가 크면 Tensor treatment가 연장되며 calibration/launch/NCU/분석 시간도 별도 |
...
V100은 B4/B16/B32 세 점만 실행한다. B4와 B16은 utilization 민감도 진단용이며,
strict coefficient는 exact NCU sidecar가 있는 B32 행을 우선한다. 이 축소로 raw row는
1,650개에서 780개로 52.7% 감소하고 nominal energy kernel 시간도 같은 비율로 줄어든다.
