...
table에도 넣지 않는다.
Tensor 값도 local raw/NCU artifact와 `observable_control_fixed_rf_v5` marker 증거가 없으므로 새 package로
RF1-16 전체를 재측정해야 한다. 과거 RTX 3090 v2 median 2.2525 pJ/FLOP도
...
`tensor_pair_kernel_revision=matched_inplace_signflip_observable_control_fixed_rf_v5`가 있어야 한다.
...
Static binary audit에서 control backward loop를 확인하고, runtime NCU에서
`SASS instructions/expected register op >= 0.1`도 통과해야 한다. v4 control은 CUDA
source에 반복문이 있어도 ptxas 후 launch-only kernel이 될 수 있었으므로 v4 Tensor
energy row는 현행 계수로 사용할 수 없다.
