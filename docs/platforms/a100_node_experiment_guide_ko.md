...
table에도 넣지 않는다.
Tensor 값도 local raw/NCU artifact와 `fixed_rf_v4` marker 증거가 없으므로 새 package로
...
`tensor_pair_kernel_revision=matched_inplace_signflip_fragment_epilogue_fixed_rf_v4`이 있어야 한다.
