...
하나만 L2 energy에 사용한다. 유효 좌표는 98개/1 repeat, `repeats=5`에서 energy raw
490행이며 Tensor pair calibration 10 coordinates/20 commands, L2 pair calibration
12 coordinates/24 commands, DRAM pair calibration 6 coordinates/12 commands, schema
smoke 3행, primary NCU 74 cases다. L2 selector는 첫 후보 통과 시 조기 종료하며
최악에는 8 candidates x 2 W x 2 modes = 32개 NCU precheck case를 추가한다.
...
| L2 CG | `global_addr_only,l2_cg_load_only` | 16,32,64,128 | NCU precheck가 B16/B8/B4 중 하나 선택 | energy load_repeat 4,8,16; 동일 pair ITER; NCU는 선택 B와 네 W 모두에서 1,2,4,8,16 |
...
모든 새 raw row는 timed kernel의 `measurement_start_epoch_ms`와
`measurement_end_epoch_ms`를 기록한다. matched-control은 완료 시각 차이였던 legacy
