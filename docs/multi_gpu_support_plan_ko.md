...
- 모든 결과 CSV는 `power_source`, `energy_source`, `ncu_version`, `ncu_supported`, `profile_name`, `compute_capability`, `sm_count`, `l2_mib`, `shared_kib_per_sm`를 포함해야 한다.
...
- 최신 Nsight Compute 13.3 문서 기준 Volta는 GPU support가 `No`이고, GA100, GA10x, GH100은 `Yes`다. 반면 Nsight Compute 2024.3 및 2025.1 release note에서는 Volta GV100이 `Yes`다. 따라서 V100 profiling은 최신 NCU 하나로 통일하면 안 되고, 지원되는 NCU 버전 또는 대체 검증 경로가 필요하다.

## GPU별 1차 지원 행렬

| GPU | uArch / chip | CC | 기본 SM 수 | L2 | shared/L1 관련 | Tensor Core baseline | 주요 주의점 |
|---|---|---:|---:|---:|---|---|---|
| RTX 3090 | Ampere GA102 / GA10x | 8.6 | 82 | 6 MiB | unified L1/shared 128 KB, shared carveout 최대 100 KB, max shared/block 99 KB, max blocks/SM 16 | FP16, TF32, BF16, INT8, INT4. FP64 Tensor Core 없음 | `blocks/SM=32` 불가. NVML `GetPowerUsage`는 1초 평균 의미. WSL에서는 NCU performance counter 권한 이슈 가능 |
| V100 | Volta GV100 | 7.0 | 80 | 6 MiB | L1/shared 통합, shared 최대 96 KB, max blocks/SM 32 | FP16 input, FP16 또는 FP32 accumulate 중심 | 최신 NCU 13.3에서는 Volta 미지원. 2024.3/2025.1 계열 NCU 또는 정적 SASS/PTX 검증 필요 |
| A100 | Ampere GA100 | 8.0 | 108 | 40 MiB | shared 최대 164 KB, max shared/block 163 KB, max blocks/SM 32 | FP16, BF16, TF32, FP64, INT8, INT4, Binary, sparsity | `GetPowerUsage`는 GA100 예외로 instantaneous 의미. MIG 사용 시 L2 set-aside 등 일부 기능 영향 |
| H100 | Hopper GH100 | 9.0 | SKU별 상이, runtime 우선 | 50 MiB | shared 최대 228 KB, max shared/block 227 KB, max blocks/SM 32, TMA/DSM/cluster | FP16/BF16/TF32 계열 + FP8, WGMMA 계열 optional | 기존 WMMA baseline은 가능하지만 Hopper 특화 실험은 WGMMA/TMA 별도 kernel 필요 |
...
- `tensor_modes`: `fp16_wmma`, `tf32`, `bf16`, `fp64_tc`, `fp8`, `int8`, `int4`, `sparsity`
- `ncu_chip_aliases`: `gv100`, `ga100`, `ga102`, `gh100` 등. 문서상의 family 이름은 GA10x지만 `ncu --list-chips`는 대개 `ga102`처럼 구체 chip명을 쓴다.
- `recommended_ncu_version`: V100은 2024.3/2025.1 계열, Ampere/Hopper는 현재 버전 가능
...
| GPU | 최신 NCU 13.3 기준 | 2024.3/2025.1 기준 | 계획 |
|---|---|---|---|
| RTX 3090 / GA10x | 지원 | 지원 | 현재 NCU 사용 가능. WSL performance counter 권한 preflight 필수 |
| V100 / GV100 | 미지원 | 지원 | V100 profiling은 NCU 2024.3 또는 2025.1 toolchain을 별도 설치/지정 |
...
- 지원되는 GPU/NCU 조합에서 stall percentage, SOL percentage, memory path percentage를 CSV/Markdown으로 요약한다.
...
- NVIDIA Nsight Compute 13.3 Release Notes
