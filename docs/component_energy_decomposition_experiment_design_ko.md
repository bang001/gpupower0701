...
| `W_SM`이 모든 mode에서 실제 working set은 아님 | `reg_mma`, `reg_operand_only`, `empty`, `store_only`, `store_path`는 sweep coordinate만 맞춘다. | `W_SM` 의존성은 memory-backed mode 중심으로만 해석해야 한다. |
...
| pair | 차분 | 추정하려는 항목 | 단위 |
|---|---|---|---|
| `reg_mma - empty` | `E_reg_mma - E_empty` | Tensor Core + register operand path baseline | J, pJ/FLOP |
| `reg_operand_only - empty` | `E_reg_operand_only - E_empty` | no-MMA register-fragment/control baseline | J, pJ/reg-op |
| `reg_mma - reg_operand_only` | `E_reg_mma - E_reg_operand_only` | Tensor Core MMA incremental cost 후보 | J, pJ/FLOP |
...
| `reg_operand_only` | Tensor Core 없는 register-fragment/control 기준 측정 | `reg_mma`와 같은 `ITER * reuse_factor` 반복 구조에서 sampled A/B fragment 값을 checksum으로 소비하되 `mma_sync`는 수행하지 않음 | `reg_mma` | expected reg operand ops, net_E_J |

`reg_fragment_only`와 `reg_operand_only`는 register/tensor 구분을 강화하기 위한 control이다. `reg_operand_only`는 pure register energy가 아니라 `mma_sync`를 제거한 no-MMA register-fragment/control baseline으로 해석한다.
...
| `reg_operand_only` | register fragment 준비 후 `reuse_factor`만큼 sampled fragment 값을 checksum으로 소비, MMA 없음 | checksum을 accumulator fragment로 store | expected reg operand ops = `active_SM * B * ITER * reuse_factor` |
...
| register/tensor | `reg_fragment_only`, `reg_operand_only`, `reg_mma` | fragment setup, no-MMA register-fragment/control, Tensor Core incremental cost 후보 분리 |
...
| 3 | register | `reg_operand_only` | no-MMA register-fragment/control |
| 4 | register | `reg_mma` | Tensor Core incremental 후보 |
| 5 | shared | `shared_load_only` | shared/L1 load only |
| 6 | shared | `shared_mma` | shared + MMA |
| 7 | L2 | `l2_load_only` | L2 load only |
| 8 | L2 | `l2_mma` | L2 + MMA |
| 9 | DRAM | `dram_load_only` | DRAM load only |
| 10 | DRAM | `dram_mma` | DRAM + MMA |
| 11 | store | `store_only` | store only |
...
|---|---|---|
| register | `reg_mma` | `empty`, `reg_fragment_only`, `reg_operand_only`, `reg_mma` |
...
```text
Delta_reg_baseline_J    = E(reg_mma, x) - E(empty, x)
Delta_reg_operand_J     = E(reg_operand_only, x) - E(empty, x)
Delta_reg_tensor_inc_J  = E(reg_mma, x) - E(reg_operand_only, x)
...
| coefficient | 계산 | 단위 |
|---|---|---|
| register/tensor baseline | `Delta_reg_baseline_J * 1e12 / FLOP` | pJ/FLOP |
| no-MMA register-fragment/control | `Delta_reg_operand_J * 1e12 / expected_reg_operand_ops` | pJ/reg-op |
| Tensor Core incremental 후보 | `Delta_reg_tensor_inc_J * 1e12 / FLOP` | pJ/FLOP |
...
| 7 | `reg_operand_only` 추가로 `reg_mma - reg_operand_only` Tensor Core incremental 후보 계산 | kernels + paired analysis | 필수 |
...
| 11 | `address_only`, `shared_init_only`, `matched_store_only` 추가 | kernels + modes | 확장 |
