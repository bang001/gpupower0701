...
## RTX 3090 2026-07-14 v5 측정 결과

2026-07-14 RTX 3090 v5 finalplan은 strict 4-component와 별도 external-memory
effective path를 완주했다.
이 값은 GA102 v5 protocol-specific accepted evidence다. 2026-07-15 A100에서 v5
control이 launch-only로 실행된 portability 실패가 확인된 후 신규 플랫폼
실행은 v6를 사용한다. 따라서 아래 숫자를 v6 계수로 재명명하거나
A100/V100/H100에 직접 복사하지 않는다.
...
Tensor v5 control은 GA102 static SASS에서 RF1/2/4/8/16 backward loop와 HMMA=0을 보이고,
...
`archive/pre_current_protocol_20260712/`에 보존한다. v2/v4/v5/v6 결과를
합산하거나 평균내지 않는다.
