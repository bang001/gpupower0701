...
반드시 docs/v100_node_experiment_guide_ko.md, docs/cross_platform_component_experiment_guide_ko.md, docs/component_energy_final_experiment_plan_ko.md, docs/ncu_validation_energy_calculation_ko.md를 먼저 읽고 진행해.
...
3. ncu --list-chips와 --query-metrics --chips gv100 확인
4. sm_70로 build
5. dry-run과 smoke run 실행
6. scripts/plan_platform_component_experiment.py로 V100 finalplan 생성
7. energy sweep 실행
8. NCU sidecar 실행. DRAM sanity는 DRAM_W_SM_KIB_OVERRIDE=8192 이상으로 확인
9. scripts/analyze_ncu_path_acceptance.py로 path acceptance 확인
10. scripts/analyze_matched_control_energy.py로 accepted row만 coefficient 계산
11. 결과 표에는 sweep 조건, 선택된 좌표, 단위, NCU hit/access/stall counter를 모두 포함
...
- NCU L1 hit, L2 hit, L1/L2/DRAM access 수, stall 비율
...
반드시 먼저 읽을 문서:
- docs/v100_node_experiment_guide_ko.md
- docs/cross_platform_component_experiment_guide_ko.md
- docs/component_energy_final_experiment_plan_ko.md
- docs/ncu_validation_energy_calculation_ko.md
- docs/component_energy_self_critique_ko.md
...
6. energy sweep
   ...
   - blocks/SM 후보는 16,32를 우선하고 필요하면 1,2,4,8도 diagnostic으로 본다.
   - Tensor는 reg_operand_only와 reg_mma matched-control 차분으로 본다.
   - shared scalar는 clocked_empty와 shared_scalar_load_only 차분으로 본다.
   - global L1은 clocked_empty와 global_l1_load_only 차분으로 본다.
   - L2는 clocked_empty와 l2_cg_load_only 차분으로 본다.
   - DRAM은 clocked_empty와 dram_cg_load_only를 sanity로 보고, W_SM은 8192 KiB 이상으로 시작한다.
...
8. path acceptance
   ...
   - L2 CG: L1 hit <= 1%, L2 hit >= 95%, DRAM/L2 bytes <= 2% 수준이어야 한다.
...
|---|---|---|---|---|
| Tensor | `reg_mma - reg_operand_only` | W_SM 2048 KiB, blocks/SM 16/32, reuse sweep | HMMA 존재, spill/local 0 | pJ/FLOP |
| Shared/L1 | `shared_scalar_load_only - clocked_empty` | W_SM 32/64 KiB, blocks/SM 16/32, load_repeat sweep | shared bytes 존재, bank conflict 낮음 | pJ/bit, pJ/Byte |
| Global L1 | `global_l1_load_only - clocked_empty` | W_SM 8/16 KiB, blocks/SM 16/32, load_repeat sweep | L1 hit >= 95%, L2/DRAM 낮음 | pJ/bit, pJ/Byte |
| L2 | `l2_cg_load_only - clocked_empty` | W_SM 64 KiB, blocks/SM 16/32, load_repeat sweep | L1 hit <= 1%, L2 hit >= 95%, DRAM 낮음 | pJ/bit, pJ/Byte |
| DRAM sanity | `dram_cg_load_only - clocked_empty` | W_SM >= 8192 KiB, blocks/SM 16/32 | DRAM bytes 충분, L2 hit 낮음 | pJ/bit, pJ/Byte 후보 |
