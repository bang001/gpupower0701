...
확인한다. `L2 miss bytes`와 `DRAM read bytes`도 함께 기록해 58-60% 같은 값이 실제
...
L2 residency policy가 기록된다.

A100 targeted L2 재실험은 normal policy의 W16/W128 KiB/SM, LR4를 먼저 검사한다.
normal에서 path-specific L2 read hit 95%를 충족하지 못할 때만 CUDA persisting-L2
access-policy window를 같은 좌표에 적용한다. persisting 결과는 default cache가 아니라
residency-managed L2 path의 effective coefficient이며, NCU hit-rate와 sector-conservation
gate는 그대로 유지한다. persisting을 선택한 row는 NCU launch metadata의 실제
