...
Memory path runner는 mode마다 목표 시간에 맞게 ITER를 따로 calibrate한다. 따라서 memory
pair는 control energy를 power rate로 바꾼 뒤 treatment elapsed time만큼 보정한다.
...
Tensor final pair는 이 방식의 예외다. `reg_mma`와 `reg_operand_only`를 따로 duration
...
분리되지 않는다. matched detail의 `pair_start_distance_ms`는 두 raw `run_id` timestamp의
차이이며 command 개수가 아니다. A100 targeted Tensor gate는 이 값이 30,000 ms 이하인지
추가로 확인한다.
