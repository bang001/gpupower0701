...
| Tensor control | `reg_operand_only`에서 Tensor/HMMA instruction = 정확히 0, spill/local 0, static backward loop >0, runtime SASS/expected register op >=0.1, treatment와 동일 NCU 좌표 accepted |
...
만들지 않는다.
또한 control HMMA=0만으로는 충분하지 않다. v4에서는 ptxas가 control 반복문 전체를
제거해도 HMMA=0/spill=0 조건이 통과했다. 현행 v5는 energy sweep 전 static SASS의
backward branch를 확인하고, NCU에서 총 SASS instruction이 expected register operation에
비례하는지 확인한다.
...
loop까지 전부 제거했다. v5는 sink를 output에 기록하며 static backward-loop audit와
runtime SASS/register-op gate를 모두 통과해야 한다. 소스에 loop가 보인다는 사실만으로
실행됐다고 판단하지 않는다.
...
`HMMA/logical MMA=4`가 되었고 RF1의 2와 불일치했다. v5는 v4의 in-place sign flip을
유지하면서 no-MMA control sink를 output에 연결한다. Treatment의 predicated HMMA를
