...
두 실행 순서의 valid row를 모두 요구한다. 현재 raw CSV는 timed kernel 직전/직후의
`measurement_start_epoch_ms`, `measurement_end_epoch_ms`를 기록한다. 분석기는 두
...
L2는 lookup hit/miss-derived 비율 하나만으로 승인하지 않는다. A100은 NCU native
op-read hit ratio와 derived ratio가 모두 95% 이상이고 차이가 2 percentage points
이하여야 한다. V100/GV100에서 native metric이 없으면 이를 가짜 통과값으로
채우지 않고 derived hit/miss, sector 보존, expected bytes, DRAM leakage를 필수
증거로 사용한다.
...
| V100 | 2048 | 32,64 | 8,16,32 | 32,64 with `l2_cg_load_only` | 8192 | Tensor/shared/L1/DRAM energy 4,16,32; L2는 NCU precheck가 normal policy의 B32/B16/B4 중 선택 |
| A100 | 2048 | 64,128 | 16,32 | 16,32,64,128 with `l2_cg_load_only` | 8192 | Tensor/shared/L1/DRAM energy 16,32; L2는 NCU precheck가 normal/persisting, layout, B16/B8/B4 중 선택 |
...
생성하지 않는다. A100은 W16/W128에서 normal 후보 뒤에 지원되는 persisting 후보도
검사한다. 어느 후보도 derived hit, 플랫폼에서 필요한 native 교차검증,
L1 bypass, sector conservation,
observed/expected traffic, DRAM leakage gate를 모두 통과하지 못하면 L2 전력 sweep을
시작하지 않는다. 95% hit 기준을 58-72%로 낮추지 않는다.
