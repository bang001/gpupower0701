...
`HMMA/logical MMA=4`가 되었고 RF1의 2와 불일치했다. v4 in-place sign flip은 SASS의
predicated HMMA를 금지하며 RTX 3090 실측에서 RF1/2/4/8/16 모두 비율 2.0을 보였다.
...
GA100은 같은 bundle에 LTC-fabric read/hit/miss를 추가해 source와 fabric 보존성을 모두
요구한다. Tensor/Shared/Global-L1/DRAM은 별도 `full`
profile로 수집한 뒤 두 CSV를 행 단위로 merge한다. 서로 다른 replay의 metric을 한 행에
섞지 않는다.
...
|---|---:|---:|---:|---:|---:|---|
| RTX 3090 | 2048 | 32,64 | 8,16 | 64 with `l2_cg_load_only` | 256,512,1024,2048 | energy 8,16; strict NCU 8 |
| V100 | 2048 | 32,64 | 8,16,32 | 32,64 with `l2_cg_load_only` | 256,512,1024,2048 | energy 4,16,32; L2는 NCU precheck가 B32/B16/B4 중 선택 |
| A100 | 2048 | 64,128 | 16,32 | 16,32,64,128 with `l2_cg_load_only` | 2048,4096,8192 | energy 16,32; L2는 NCU precheck가 B16 우선, B8/B4/B2/B1 fallback |
| H100 | 2048 | 64,128 | 16,32 | 64,128 with `l2_cg_load_only` | 2048,4096,8192 | 16,32 |
...
| Shared scalar | `shared_scalar_addr_only,shared_scalar_load_only` | energy `load_repeat=4,8,16`; NCU 1,2,4,8,16; 동일 ITER |
...
| External-memory read path | `global_addr_only,dram_cg_load_only` | architecture별 W sweep; energy `load_repeat=4,8,16`; NCU 1,4,8,16 |
