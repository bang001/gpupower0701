...
| L2 CG hit path | path-specific L1 hit bytes, L2 source read hit/miss, architecture-specific fabric/remote lookup, native hit, DRAM read | `.cg` 요청은 L1TEX를 통과하되 L1 cache hit는 거의 없어야 한다. GA100은 첫 partition miss와 최종 L2 miss를 구분한다. |
...
GA100 row에는 추가로 `l2_fabric_read/hit/miss_sectors`,
...
값을 재구성하고 절대 오차 <=2 percentage points를 요구한다. RTX 3090/H100은 각
architecture에서 검토한 direct/native 교차 gate를 유지한다. V100에서는 unavailable
...
`ncu_metric_profile`, `l2_residency_policy`, `l2_address_layout`을 남긴다. A100/V100 새 finalplan은 긴 energy
...
GA100 bundle은 여기에 `srcunit_ltcfabric` read/hit/miss를 같은 replay로 추가한다.
...
| L2 CG | `ncu_metric_profile=l2_path_minimal`. GA100은 `(source hit + fabric hit)/source read >=95%`, source/fabric sector 보존 0.98-1.02, coherent routing, native-fabric-model 오차 <=2 pp를 요구한다. 다른 profile은 검토된 direct/native 정책을 사용한다. 공통으로 observed/expected source L2 bytes=0.95-1.05, L1 path hit <=1%, L1 hit/request bytes <=1%, DRAM-read/source-L2-read <=2%가 필요하다. GV100은 native metric만 생략 가능하며 aggregate/full-bundle hit rate는 진단값이다. |
| External-memory read path | L1 hit <=1%, final-service L2 hit <=10%(A100은 source+fabric logical hit), source/expected bytes 0.95-1.05, external read/expected 0.85-1.05, external read/source read >=0.90, write/read <=0.01, direct read/write byte-counter provenance |
