...
| `DRAM = 3.542 pJ/bit`는 GDDR6X device energy다 | cache, memory controller, stall, board-level 효과가 섞인다 | `DRAM CG streaming sanity path` |
...
| DRAM CG streaming path, W_SM=8192 KiB | 3.542 | pJ/bit | 2.964-4.454 | medium-high | sanity candidate only |
...
| L2 LR8 paired 30초 auxiliary도 0.960 pJ/bit로 current primary 1.017 pJ/bit와 같은 order다. | L2 SRAM array 단독 energy가 0.96-1.03 pJ/bit라고 주장할 수 없다. |
...
| L1 paired 30초 combined run이 0.148 pJ/bit, 12/12 valid로 안정화되어 current primary로 승격됐다. | drift-sensitive path에는 factor sweep보다 paired/bracketed control 설계가 더 적합하다. |
