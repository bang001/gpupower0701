...
| DRAM increment over L2 | 과거 ordered-fit 21.180 pJ/bit | historical model component이며 현행 strict coefficient가 아님 |
| DRAM streaming cumulative path | **26.709-28.409 pJ/bit** | 최신 provisional reporting band. path 전체 effective 범위이며 HBM2 device 3.9 pJ/bit와 직접 비교 금지 |

이 범위는 GPUJoule의 K40 transaction-path 값과 magnitude 비교는 가능하지만, HBM2
device 값 또는 Horowitz SRAM 값과 같은 표에서 "더 크다/작다"로 해석하면 안 된다.
또한 현행 matched-ITER raw pair가 없으므로 literature consistency가 measurement
