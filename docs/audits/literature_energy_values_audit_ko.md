...
### 4. 우리의 RTX 3090 protocol-specific/historical 결과와 문헌값 비교

2026-07-14 RTX 3090 v5 package는 GA102에서 당시 protocol을 통과했다. A100
v5 portability 실패 후 신규 플랫폼 실행은 v6를 사용하므로, v5 값은
historical GA102 accepted evidence로 보존하고 v6와 평균하지 않는다.

| 항목 | 값 | 현재 지위와 올바른 해석 |
|---|---:|---|
| Tensor MMA incremental v5 | **2.140 pJ/FLOP** | historical GA102 effective coefficient; unmatched register/scheduler path 포함 |
| Shared scalar path | **0.714 pJ/bit** | 20260714 GA102 strict accepted effective path |
| Global L1 hit path | **0.852 pJ/bit** | 20260714 GA102 strict accepted effective path |
| L2 CG hit path | **9.078 pJ/bit** | 20260714 GA102 strict accepted effective path; pure L2 SRAM energy 아님 |
| RTX 3090 external-memory read path | **24.949 pJ/bit** | 20260714 GA102 accepted effective path; physical GDDR6X energy 아님 |
| Tensor v2/v4와 2026-07-08 memory | historical | superseded/rejected; 현행 인용 금지 |
...
Current RTX 3090 값은 GPUJoule의 K40 transaction-path 값과 magnitude context를
논의할 수는 있지만, HBM2 device 값 또는 Horowitz SRAM 값과 같은 측정 경계로
"더 크다/작다"를 판정하면 안 된다. Literature consistency는 현행 address-control raw
pair와 exact-coordinate NCU acceptance를 대신하지 않는다.
