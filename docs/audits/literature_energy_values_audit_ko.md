...
| Our effective coefficient | NVML GPU/device energy 차분을 logical FLOP 또는 NCU actual traffic으로 나눈 계수 | RTX 3090 fixed-RF v2 Tensor와 historical memory-path coefficient |
...
현행 protocol을 모두 통과한 RTX 3090 memory component table은 아직 없다. 현재
standalone 결과는 fixed-RF v2 Tensor뿐이고, memory 숫자는 historical/provisional로
제한해서 읽어야 한다.

| 항목 | 값 | 현재 지위와 올바른 해석 |
|---|---:|---|
| Tensor MMA incremental | **2.252501 pJ/FLOP** | current standalone fixed-RF v2 board-level effective coefficient; memory pJ/bit 문헌과 직접 비교하지 않음 |
...
| DRAM streaming cumulative path | **26.709-28.409 pJ/bit** | provisional reference-aligned reporting band; accepted raw coefficient가 아니며 HBM2 device 3.9 pJ/bit와 직접 비교 금지 |

Historical memory 값과 DRAM band는 GPUJoule의 K40 transaction-path 값과 magnitude
