...
2026-07-14 RTX 3090 v5 package는 현행 protocol을 통과했다. 과거 fixed-RF v2/v4와
2026-07-08 memory 숫자는 historical/provisional로만 남기고 current 결과와 평균하지
않는다.

| 항목 | 값 | 현재 지위와 올바른 해석 |
|---|---:|---|
| Tensor MMA incremental v5 | **2.140 pJ/FLOP** | current effective coefficient; unmatched register/scheduler path 포함 |
| Shared scalar path | **0.714 pJ/bit** | current strict accepted effective path |
| Global L1 hit path | **0.852 pJ/bit** | current strict accepted effective path |
| L2 CG hit path | **9.078 pJ/bit** | current strict accepted effective path; pure L2 SRAM energy 아님 |
| RTX 3090 external-memory read path | **24.949 pJ/bit** | current accepted effective path; physical GDDR6X energy 아님 |
