...
작성일: 2026-07-03, updated 2026-07-14
...
| Our effective coefficient | NVML GPU/device energy 차분을 logical FLOP 또는 NCU actual traffic으로 나눈 계수 | superseded RTX 3090 fixed-RF v2 Tensor와 historical memory-path coefficient |
...
### 4. 우리의 RTX 3090 current/historical 결과와 문헌값 비교

현행 protocol을 모두 통과한 RTX 3090 component table은 아직 없다. fixed-RF v2
Tensor와 memory 숫자는 historical/provisional로 제한해서 읽어야 하며, v4 Tensor는
경로 검증 후 새 board-energy run을 기다리는 상태다.

| 항목 | 값 | 현재 지위와 올바른 해석 |
|---|---:|---|
| Tensor MMA incremental v2 | **2.252501 pJ/FLOP** | superseded historical board-level coefficient; accumulator 정체 가능성 때문에 현행 인용 금지 |
| Tensor MMA incremental v4 | - | runtime NCU/FLOP 검증만 완료; 새 board-energy run 필요 |
| Shared scalar path | 0.170590 pJ/bit | 2026-07-08 historical coefficient; full current package 재실행 필요 |
| Global L1 hit path | 0.173483 pJ/bit | historical/provisional; 현행 `global_addr_only` control evidence 없음 |
| L2 CG hit path | 1.131073 pJ/bit | historical/provisional; 현행 matched-ITER address-control evidence 없음 |
| RTX 3090 external-memory read path | **25.510 pJ/bit** | 사용자 전달 historical effective-path 후보; GDDR6X board path이며 strict raw package 미확보 |
| A100 external-memory read path | **11.925 pJ/bit** | 사용자 전달 historical effective-path 후보; HBM2 device 3.97 pJ/bit와 measurement scope가 다름 |
| V100 external-memory read path | **8.131 pJ/bit** | 사용자 전달 historical effective-path 후보; HBM2 device 3.97 pJ/bit와 measurement scope가 다름 |

Historical memory 값과 external-memory observation은 GPUJoule의 K40 transaction-path 값과 magnitude
context를 논의할 수는 있지만, HBM2 device 값 또는 Horowitz SRAM 값과 같은 표에서
"더 크다/작다"로 판정하면 안 된다. 현행 address-control raw pair와 exact-coordinate
NCU evidence가 없으므로 literature consistency가 measurement acceptance를 대신하지
않는다.
...
| 4 | 현행 RTX 3090 memory package 재실행 | address control과 exact-coordinate NCU actual bytes를 같은 W/B/LR 좌표에 연결 |
