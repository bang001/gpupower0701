...
성립하지 않는다. 같은 실행의 L2 path-specific read hit는
...
| 후속 A100 L2 path hit | path-specific L2 read hit 58.5-60.1%, L1 bypass는 통과 | W16-128의 logical working set은 1.688-13.5 MiB로 40 MiB보다 작고 hit가 W와 무관하게 약 60%다. 단순 capacity miss보다는 power-of-two block-private stride의 L2 set/slice/partition 집중, replay/cache state 또는 counter 분모 문제가 우선 의심된다. 또한 persisting window 검증에 필요한 `launch__persisting_l2_cache_size`를 summarizer/selector는 요구했지만 NCU 기본 metric 목록이 요청하지 않은 구체적 버그가 있었다. | 누락 metric을 수집하고 application replay에서 CG warm-up을 pass마다 재실행한다. `L2 read bytes / expected logical bytes=0.95-1.05`를 새 hard gate로 추가한다. 95% hit 기준은 낮추지 않고 policy/layout/blocks-SM 후보를 순서대로 검사한다. 전부 실패하면 energy 전에 중단한다. |
...
| L2 CG | W_SM 16,32,64,128 KiB, selected blocks/SM 16, 8 또는 4 | LR 4,8,16 | 각 W에서 LR 1,2,4,8,16 | derived/native L2 read hit 모두 >=95%, 두 hit rate 차이 <=2 percentage points, hit+miss/total read sectors=1+/-2%, observed L2 read bytes/expected logical bytes=0.95-1.05, L1 path hit <=1%, L1 hit/request bytes <=1%, DRAM/L2 read <=2% |
...
새 package의 실행 순서는 다음과 같다. 후보 순서는 사전 고정하며 95% 기준을 낮춰 통과점을
만들지 않는다.

1. `normal/contiguous/B16`, W16/W128 KiB/SM, LR4를 NCU로 먼저 측정한다.
...
3. 모든 normal 후보가 실패하면 같은 순서로 `persisting/contiguous/B16`과
   `persisting/sm_interleaved/B16,B8,B4`를 검사해 residency 정책 효과를 분리한다.
4. persisting API/metric이 지원되지 않고 모든 normal 후보가 실패했다면 strict L2 결과 없이
   ...
   변경하지 않는다.
5. 각 후보는 W16/W128의 treatment/control 네 행이 모두 L1 bypass, derived/native L2 hit
   95%, sector 보존, DRAM/L2, observed/expected traffic gate를 통과해야 선택된다.
6. 어떤 후보도 통과하지 못하면 coefficient를 계산하지 않고 종료한다.
7. 선택된 policy/layout/blocks-SM으로 Tensor/L2 full NCU를 실행하고 precheck를 통과시킨
...
긴 sweep 전에 후보당 L2 NCU 4 cases를 실행하며 첫 통과 후보에서 멈춘다. 최선은 4 cases,
8개 후보가 모두 실패하면 최대 32 cases 후 energy 없이 종료한다. 구성이 선택된 경우에만
...
| A100 finalplan dry-run | NCU selector가 고른 B 하나에서 W16/32/64/128 L2 energy 24 commands/repeat, L2 dual-calibration 12 coordinates/24 calibration commands, primary NCU 74 cases와 최대 32-case selector precheck 확인 | 실제 A100 cache hit/energy coefficient는 아직 재측정 전; selector가 두 anchor에서 통과하지 않으면 energy 전에 종료 |
