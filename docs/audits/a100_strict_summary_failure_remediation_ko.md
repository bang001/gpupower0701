...
작성일: 2026-07-10, updated 2026-07-11
...
| L2 CG | W_SM 16,32,64,128 KiB, blocks/SM 16 | LR 4,8,16 | 각 W에서 LR 1,2,4,8,16 | path-specific L2 read hit >=95%, L1 path hit <=1%, L1 hit/request bytes <=1%, DRAM/L2 read <=2% |
...
`L2 read path hit >=95%`의 동시 조건으로 검증한다.
...
| `verify_smid` atomic/background가 L1/L2 counter에 혼입 | SMID 기록은 block당 atomic 1회다. aggregate L2에는 보일 수 있지만 path-specific L1 global-load lookup과 workload 규모 대비로 분리한다. control에서도 동일 bookkeeping을 실행한다. | aggregate counter 오염 가능성은 있음. sector 0을 요구하지 않고 exact op/read lookup counter 사용 |
...
RF1-16 dual calibration 10 commands와 pair 70 energy rows, L2 W16-128/LR4-16 pair 168 energy rows, Tensor/L2 NCU
50 cases를 수행하고 전용 audit가 RF별 양수 차분과 L2 path/plateau를 모두 판정한다.
...
`tensor_pair_kernel_revision=matched_add_scalar_epilogue_v1`과
...
| 검증 | 결과 | 한계 |
|---|---|---|
| 로컬 CUDA build | CUDA 13.2에서 sm_86/sm_80 clean build 모두 성공. 최종 scalar epilogue 기준 sm_80 ptxas `reg_operand_only=22`, `reg_mma=26 registers/thread`, 둘 다 spill 0 | A100 runtime/clock/power 결과는 target node에서 재확인 필요 |
...
| Pair adjacency gate | Tensor 및 Shared/L1/L2/DRAM의 알려진 2-mode pair를 command 단위가 아니라 pair 단위로 회전 | runner matrix와 matched detail의 `pair_start_distance_ms<=30000` 확인; target A100 재실행 필요 |
| Targeted result audit | RF별 dual-calibration max ITER/control duration/HMMA/spill/delta와 W별 path hit/energy plateau를 `audit_a100_tensor_l2_remediation.py`가 검사 | synthetic pass, bad max policy, short control, RF4 negative, L2 path 72% reject self-test 통과; 실제 A100 artifact 필요 |
