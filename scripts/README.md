...
`coefficient_eligible`와 함께 `sweep_source_id`, 물리 `gpu_id`, `n_gpu_active`,
`gpu_active`가 있어야 한다. package audit은 `status=reject`,
...
동일 `run_id`에 여러 GPU row가 있는 것은 repeat가 아니라 멀티 GPU 관찰 행이다.
후단 조인은 `(sweep_source_id, run_id, gpu_id)`를 사용하며, 구형 audit CSV에
`gpu_id`가 없으면 raw 5개 파일에서 audit을 다시 생성해야 한다.
...
5개 raw CSV를 한 번에 넘겨도 pairing은 `sweep_source_id`별로 격리된다. 특히
L1/L2/DRAM sweep의 동일 좌표 `global_addr_only`는 서로 대체할 수 없다. NCU exact
coordinate key도 label/source에서 얻은 `l1`, `l2`, `dram` family를 포함한다.
