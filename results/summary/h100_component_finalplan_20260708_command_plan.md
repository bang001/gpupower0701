...
| NCU sudo fallback | `NCU_USE_SUDO=1 bash results/summary/h100_component_finalplan_20260708_commands.sh` |
| generated shell | `results/summary/h100_component_finalplan_20260708_commands.sh` |
...
| L2 | `global_addr_only,l2_cg_load_only` | 64,128 | 64/16 | energy load_repeat 4,8,16; treatment/control-floor dual-calibrated pair-locked ITER; NCU also checks 1,2 |
...
H100 uses `NCU_CHIP=gh100` and `l2_cg_load_only` at
W_SM=64,128 KiB for the L2 candidate sweep. The current kernels use
...
```bash
bash results/summary/h100_component_finalplan_20260708_commands.sh
```
...
```bash
NCU_BIN="$(command -v ncu)" NCU_USE_SUDO=1 NCU_SUDO="sudo -E" bash results/summary/h100_component_finalplan_20260708_commands.sh
```
