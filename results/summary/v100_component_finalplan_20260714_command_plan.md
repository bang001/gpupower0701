...
| NCU sudo fallback | `NCU_USE_SUDO=1 bash results/summary/v100_component_finalplan_20260714_commands.sh` |
...
| generated shell | `results/summary/v100_component_finalplan_20260714_commands.sh` |
...
| Shared scalar | `shared_scalar_addr_only,shared_scalar_load_only` | 32 | 32/32 | energy and NCU load_repeat 4,8,16; dual-calibrated equal ITER |
| Global L1 | `global_addr_only,global_l1_load_only` | 32 | 32/32 | energy and NCU load_repeat 4,8,16; dual-calibrated equal ITER |
| L2 | `global_addr_only,l2_cg_load_only` | 32,64 | 32,64/selected by NCU precheck | energy and final NCU load_repeat 4,8,16; selector probes LR4; treatment/control-floor dual-calibrated pair-locked ITER |
| External-memory read path (effective) | `global_addr_only,dram_cg_load_only` | 256,512,2048 | 256,512,2048/32 | W_SM은 nominal L2 배수 sweep; energy load_repeat 4,8,16; pair-locked ITER; NCU read-byte conservation/write-contamination 검증 |
...
`W32/B32`. Coordinates omitted before execution because
...
```bash
bash results/summary/v100_component_finalplan_20260714_commands.sh
```
...
```bash
NCU_USE_SUDO=1 bash results/summary/v100_component_finalplan_20260714_commands.sh
```
...
```bash
NCU_BIN="$(command -v ncu)" NCU_USE_SUDO=1 NCU_SUDO="sudo -E" bash results/summary/v100_component_finalplan_20260714_commands.sh
```
