...
| NCU sudo fallback | `NCU_USE_SUDO=1 bash results/summary/h100_component_finalplan_20260714_commands.sh` |
...
| generated shell | `results/summary/h100_component_finalplan_20260714_commands.sh` |
...
Build the benchmark for `sm_90` before running the generated
...
| L2 | `global_addr_only,l2_cg_load_only` | 64,128 | 64,128/selected by NCU precheck | energy and final NCU load_repeat 4,8,16; selector probes LR4; treatment/control-floor dual-calibrated pair-locked ITER |
...
H100 uses `NCU_CHIP=gh100`; GH100 has a partitioned L2
crossbar, so strict NCU tests the W_SM=64,128 KiB candidates before
...
```bash
bash results/summary/h100_component_finalplan_20260714_commands.sh
```
...
```bash
NCU_USE_SUDO=1 bash results/summary/h100_component_finalplan_20260714_commands.sh
```
...
```bash
NCU_BIN="$(command -v ncu)" NCU_USE_SUDO=1 NCU_SUDO="sudo -E" bash results/summary/h100_component_finalplan_20260714_commands.sh
```
