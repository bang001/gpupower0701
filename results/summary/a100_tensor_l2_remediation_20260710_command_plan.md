...
| pair가 반복 경계에서 분리될 가능성 | command 한 개가 아니라 control-treatment 2개를 원자적 pair로 회전하고 timed kernel interval을 직접 기록 | 20 s run의 runner matrix와 matched-detail `pair_transition_gap_ms<=35000`; 완료 시각 차이가 아닌 두 benchmark interval 사이 공백 |
