...
# GA100 40 MiB L2 path. Final L2 candidates sweep ld.global.cg at W_SM=16,32,64,128 KiB and require path-specific L1 hit bytes plus L2 read hit/miss evidence; l2_load_only is diagnostic-only.
...
# 1. Preflight
python3 scripts/preflight_gpu_support.py --gpu 0 --target-profile a100 --strict --active-sm 108 --binary ./build-a100/a100_fp16_energy_v2 --ncu "${NCU_COMMAND}" --nvcc "${NVCC_COMMAND}" --out results/summary/a100_component_finalplan_20260708_preflight.md
...
python3 scripts/audit_documentation_consistency.py --out-csv results/summary/a100_component_finalplan_20260708_documentation_consistency_audit.csv --out-md results/summary/a100_component_finalplan_20260708_documentation_consistency_audit.md --fail-on-error
...
STALE_PATHS=(
  ...
  results/raw/a100_component_finalplan_20260708_tensor_pair_calibration.csv
  results/raw/a100_component_finalplan_20260708_l2_pair_calibration.csv
  results/raw/a100_component_finalplan_20260708_dram_pair_calibration.csv
  results/summary/a100_component_finalplan_20260708_l2_path_selection.csv
  results/summary/a100_component_finalplan_20260708_l2_path_selection.md
  results/summary/a100_component_finalplan_20260708_l2_path_selection.env
...
shopt -s nullglob
for path in results/raw/a100_component_finalplan_20260708_l2_precheck_* results/summary/a100_component_finalplan_20260708_l2_precheck_*; do
  mkdir -p "${STALE_DIR}/$(dirname "${path}")"
  mv "${path}" "${STALE_DIR}/${path}"
done
shopt -u nullglob
...
./build-a100/a100_fp16_energy_v2 --gpu-list 0 --mode reg_operand_only --w-sm-kib 2048 --blocks-per-sm 1 --target-profile a100 --active-sm 1 --seconds 0.2 --iters 1 --repeats 1 --reuse-factor 1 --load-repeat 1 --store-repeat 1 --output results/raw/a100_component_finalplan_20260708_schema_smoke.csv --verify-smid 0
./build-a100/a100_fp16_energy_v2 --gpu-list 0 --mode l2_cg_load_only --w-sm-kib 16 --blocks-per-sm 1 --target-profile a100 --active-sm 1 --seconds 0.2 --iters 1 --repeats 1 --reuse-factor 1 --load-repeat 1 --store-repeat 1 --output results/raw/a100_component_finalplan_20260708_schema_smoke.csv --verify-smid 0
...
run_l2_path_candidate() {
  ...
  local outdir=results/ncu/a100_component_finalplan_ncu_factor_20260708/l2_precheck_${candidate}
  local raw_out=results/raw/a100_component_finalplan_20260708_l2_precheck_${candidate}.csv
  local acceptance_csv=results/summary/a100_component_finalplan_20260708_l2_precheck_${candidate}_acceptance.csv
  local acceptance_md=results/summary/a100_component_finalplan_20260708_l2_precheck_${candidate}_acceptance.md
  ...
  python3 scripts/select_l2_path_configuration.py \
    ...
    --expected-w 16,128 \
    --load-repeat 4 \
    --out-csv results/summary/a100_component_finalplan_20260708_l2_path_selection.csv \
    --out-md results/summary/a100_component_finalplan_20260708_l2_path_selection.md \
    --out-env results/summary/a100_component_finalplan_20260708_l2_path_selection.env || selector_rc=$?
  if [[ "${selector_rc}" == "0" ]]; then
    source results/summary/a100_component_finalplan_20260708_l2_path_selection.env
    export L2_BLOCKS_PER_SM L2_RESIDENCY_POLICY L2_ADDRESS_LAYOUT
    return 0
  fi
...
if [[ "${L2_PATH_SELECTED}" != "1" || -z "${L2_BLOCKS_PER_SM:-}" || -z "${L2_RESIDENCY_POLICY:-}" || -z "${L2_ADDRESS_LAYOUT:-}" ]]; then
  ...
  echo "Inspect results/summary/a100_component_finalplan_20260708_l2_path_selection.md and the l2_precheck_* NCU logs." >&2
  exit 2
fi
