...
python3 scripts/run_component_regression_sweep.py --execute --binary ./build-a100/a100_fp16_energy_v2 --target-profile a100 --gpu-ids 0 --max-active-gpus 1 --modes global_addr_only,l2_cg_load_only --w-sm-kib-values 16,32,64,128 --blocks-per-sm-values 16,32 --active-sm-values 108 --reuse-factors 1 --load-repeats 4,8,16 --store-repeats 1 --seconds 10.0 --repeats 5 --output results/raw/a100_component_finalplan_20260708_l2.csv --matrix-csv results/raw/a100_component_finalplan_20260708_l2_matrix.csv --memory-pair-lock-iters --memory-pair-control-min-seconds 1.0 --memory-pair-calibration-csv results/raw/a100_component_finalplan_20260708_l2_pair_calibration.csv
...
NCU_EXPLICIT_METRICS_ONLY=1 NCU="${NCU_BIN}" NCU_USE_SUDO="${NCU_USE_SUDO}" NCU_AUTO_SUDO="${NCU_AUTO_SUDO}" NCU_SUDO="${NCU_SUDO}" BIN=./build-a100/a100_fp16_energy_v2 OUTDIR=results/ncu/a100_component_finalplan_ncu_factor_20260708 RAW_OUT=results/raw/a100_component_finalplan_ncu_factor_20260708.csv TARGET_PROFILE=a100 NCU_CHIP=ga100 NCU_FILTER_UNAVAILABLE_METRICS=1 GPU=0 ACTIVE_SM=108 BLOCKS_PER_SM=16 REG_BLOCKS_PER_SM=16 REG_PRESSURE_PAYLOAD_BYTES=256 REG_W_SM_KIB=2048 L1_W_SM_KIB=16 SHARED_W_SM_KIB=128 L2_W_SM_KIB=16 L2_W_SM_KIB_VALUES=16,32,64,128 DRAM_W_SM_KIB_OVERRIDE=8192 INCLUDE_L2_CAPACITY_NCU=0 INCLUDE_DIAGNOSTIC_NCU=0 REUSE_FACTOR=1 LOAD_REPEAT=1 TENSOR_REUSE_FACTORS=1,2,4,8,16 MEMORY_LOAD_REPEATS=1,2,4,8,16 DRAM_LOAD_REPEATS=1,4,8,16 bash scripts/run_ncu_validation.sh
