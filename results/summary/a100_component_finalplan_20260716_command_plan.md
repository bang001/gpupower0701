...
| NCU sudo fallback | `NCU_USE_SUDO=1 bash results/summary/a100_component_finalplan_20260716_commands.sh` |
...
| generated shell | `results/summary/a100_component_finalplan_20260716_commands.sh` |
...
Build the benchmark for `sm_80` before running the generated
...
prints `REAL GPU CALIBRATION: profile=a100` and
...
A100 uses `NCU_CHIP=ga100`; its L2 candidates are below the
40 MiB L2 capacity and use `ld.global.cg` to avoid global-L1
...
```bash
bash results/summary/a100_component_finalplan_20260716_commands.sh
```
...
```bash
NCU_USE_SUDO=1 bash results/summary/a100_component_finalplan_20260716_commands.sh
```
...
```bash
NCU_BIN="$(command -v ncu)" NCU_USE_SUDO=1 NCU_SUDO="sudo -E" bash results/summary/a100_component_finalplan_20260716_commands.sh
```
